...
14. For frontend/UI work, inspect `frontend/src/components/shared/` before creating a generic control. Reuse a RISpro shared primitive when its semantics fit; do not add a second generic Button, Input, Card, Table, Dialog/modal shell, Badge, LoadingState, EmptyState, or ErrorState without a documented reason. Reuse existing tokens, preserve behavior and established layout, and validate relevant desktop/mobile UI in a browser. Domain-specific controls may remain local when their semantics genuinely differ; see `docs/agents.md`.
15. Any task modifying frontend application code must run both `npm run typecheck:frontend` and `npm run build:frontend` after the final frontend code edit, against the exact final working tree intended for commit/push. If frontend code changes after either command, rerun both. Vitest, lint, or successful transpilation are not substitutes for the real TypeScript project build. Never report frontend build/typecheck as passing unless the command completed successfully on that final tree; if either cannot run, report it as blocked/not run, never passed.
