...
6. Set label dimensions (50 x 30 mm initially) and receipt roll width (80 mm initially), configure matching custom media in the Windows driver, and enable per-profile rasterization only if that driver requires it. Orientation is read-only and normally derived from final physical dimensions. Standard A4 remains 210 x 297 mm with `custom:false`; registration-list jobs reuse that A4 profile with a job-only landscape orientation. Appointment slips retain their normal A4/A5 behavior. Test print generates an exact-size PDF with the same orientation used by QZ and passes through normal queue validation, signing, duplicate protection, timeouts, and client-reported audit; run it, then save.
