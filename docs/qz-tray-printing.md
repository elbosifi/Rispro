...
- Appointment A4/A5 PDFs are rendered server-side by Chromium in the main RISpro application container. The existing browser appointment-slip HTML/CSS is the canonical presentation; normal browser printing continues to use that HTML unchanged. Accession labels use a 50 x 30 mm jsPDF document by default, printer tests use an exact-size jsPDF page, and patient PDFs reuse `/api/documents/:documentId/view`.
