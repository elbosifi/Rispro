...
5. Refresh printers and select the exact OS queue returned by QZ for A4 Portrait, A4 Landscape, A5, accession label, and receipt profiles. A4 Portrait and A4 Landscape are independent RISpro workstation profiles: map Portrait to a Windows queue whose defaults are A4 + Portrait, and Landscape to a Windows queue whose defaults are A4 + Landscape. Both Windows queues may target the same physical printer and port. This driver-level setup is intentional because Windows/QZ landscape output depends on the queue's driver defaults. If a driver requires a particular tray, enter its exact Windows tray name manually; otherwise leave the field empty for the printer default.
6. Set label dimensions (50 x 30 mm initially) and receipt roll width (80 mm initially), configure matching custom media in the Windows driver, and enable per-profile rasterization only if that driver requires it. Orientation is read-only and derived from physical dimensions. A4 Portrait is 210 x 297 mm; A4 Landscape is 297 x 210 mm; both use `custom:false`. Registration lists, landscape Report Center output, Statistics, and A4 Landscape test prints use the Landscape mapping. Portrait A4 documents use the Portrait mapping; A5 remains unchanged. Test print generates an exact-size PDF with the same orientation used by QZ and passes through normal queue validation, signing, duplicate protection, timeouts, and client-reported audit; run it, then save.
