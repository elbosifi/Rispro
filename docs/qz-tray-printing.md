...
- `/api/printing/qz-certificate`, `/api/printing/qz-sign`, and `/api/printing/runtime-config` are authenticated and limited to printing roles. The signer allowlist contains only QZ Tray 2.2.6 calls `printers.find`, `printers.detail`, and `print`; signed print calls must contain only `pixel` / `pdf` / `base64` data for a named local printer. HTML, files, URLs, raw print languages, file/socket/serial/USB/HID calls, and unknown calls are rejected. The private key is used only by the server. `/api/printing/audit` stores validated client-reported metadata, never document content.

For `printers.find` and `print`, RISpro builds the exact QZ 2.2.6 request with one timestamp, computes its SHA-256 digest, obtains the server's RSA SHA-512 signature, and passes that signature and identical timestamp into QZ's explicit pre-signing parameters. RISpro never overrides `Date.now()`. QZ 2.2.6 `printers.details()` exposes no signature or timestamp parameters, so driver-detail/tray discovery is not invoked in deterministic mode; existing saved tray mappings remain usable. The backend retains the narrowly validated `printers.detail` call for compatibility, but the current frontend does not request it.
...
`QZ_SIGNING_REQUEST_LIMIT_MB` limits the UTF-8 bytes of the inner signed request without raising RISpro's global parser limit. At 25 MiB, Base64 expansion leaves approximately 18.75 MiB for PDF bytes before small JSON/configuration overhead. RISpro sends the exact deterministic request alongside its digest, recomputes SHA-256 server-side, validates the complete request, then signs the verified digest with RSA SHA-512.
...
5. Refresh printers and select the exact OS queue returned by QZ for A4, A5, accession label, and receipt profiles. Existing saved tray mappings are retained; see the QZ 2.2.6 deterministic-signing limitation below before changing a tray mapping.
6. Set label dimensions (50 x 30 mm initially) and receipt roll width (80 mm initially), configure matching custom media in the Windows driver, and enable per-profile rasterization only if that driver requires it. A4 and A5 remain standard vector PDF jobs by default. Test print generates an exact-size PDF and passes through normal queue validation, signing, duplicate protection, timeouts, and client-reported audit; run it, then save.
...
- QZ tray/status capabilities vary by Windows driver. Deterministic QZ 2.2.6 mode cannot refresh driver tray details because `printers.details()` lacks explicit signing arguments. RISpro records submission acceptance, not physical completion.
