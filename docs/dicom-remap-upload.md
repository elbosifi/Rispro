...
- **Recover Source** is a separate failed-job recovery export. While private staging remains retained, the authenticated job owner can download `GET /api/pacs/remap/jobs/:jobId/recover-source`. RISpro revalidates the manifest, hashes, sizes, and confirmed selected-study membership before streaming a ZIP of neutral `000001.dcm` entries. Its bytes are the original private staged source only: no remapped output, Orthanc content, or delimiter repair is used. Downloading does not retry, reset, or otherwise change the remap job; **Retry with Orthanc** remains the separate recovery action.
