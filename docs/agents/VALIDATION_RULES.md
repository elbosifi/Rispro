...
Do not spend Codex quota troubleshooting, installing, or provisioning Docker solely to run the full DB suite. Delegate that suite to the required GitHub CI workflow and report exactly: `DB-backed validation delegated to GitHub CI and remains pending.` A green required GitHub CI check is the authoritative full-suite result before merge or deployment gating.
