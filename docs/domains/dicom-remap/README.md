...
- Staged DICOM is PHI and must remain outside public static serving. Use `DICOM_REMAP_STAGING_DIR` on the persistent RISpro storage volume. Successful sends retain pristine source staging for the configurable DICOM Remap Source Retention period in Settings > PACS (default four days), keeping Recover Source available while staging validates. The periodic worker cleans it after expiry; reducing retention can clean older retained sources on the next cycle and cannot restore already-deleted files. Failed and awaiting-confirmation retention remain separate. Explicit safe cancellation/reset still removes staging.
...
- **Recover Source** is a status-independent export of finalized private staging for authorized DICOM Remap users whenever that staging remains retained and validates. It streams only the confirmed selected source study after manifest, size, SHA-256, and DICOM Study Instance UID validation. ZIP entries are neutral sequential names; their contents are pristine staged bytes, never remapped, Orthanc-modified, or delimiter-repaired working copies. A download does not extend retention or mutate the job; it becomes unavailable once the retention worker securely removes staging.
