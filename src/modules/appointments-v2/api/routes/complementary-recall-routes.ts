...
import { queueManualComplementaryRecallCompletedEmail } from "../../recall/complementary-recall-email.js";
...
complementaryRecallRouter.post("/:id/send-completion-email", asyncRoute(async (req: RecallRequest, res: Response) => { const result = await withTransaction((client) => queueManualComplementaryRecallCompletedEmail(client, { recallRequestId: id(req.params.id), actorUserId: Number(req.user!.sub), forceResend: req.body?.forceResend === true })); res.status(202).json(result); }));
