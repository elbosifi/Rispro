...
describe("Doctor Portal wiring", () => {
  ...
  it("keeps Settings Users doctor profile changes delegated to Doctor Directory", () => {
    const source = readFileSync(join(rootDir, "frontend", "src", "pages", "settings", "users-section.tsx"), "utf8");
    assert.match(source, /doctor\/doctors-directory\?linkUserId=\$\{selectedUser\.id\}/);
    assert.match(source, /settings\.users\.configureDoctorProfile/);
