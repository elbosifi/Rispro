...
router.post("/appointments/:appointmentId/complementary-recalls", asyncRoute(async (req: DoctorRequest, res: Response) => {
  ...
  const recall = await withTransaction((client) => createComplementaryRecall(client, { originalAppointmentId: positiveInteger(req.params.appointmentId, "appointmentId"), receptionInstruction: optionalText(body.receptionInstruction ?? body.reception_instruction), technologistInstruction, reasonCode: body.reasonCode ?? body.reason_code, qaClassification: body.qaClassification ?? body.qa_classification, urgency: body.urgency, dueAt: body.dueAt ?? body.due_at, reportingDisposition: body.reportingDisposition ?? body.reporting_disposition, requestedModalityId: body.requestedModalityId ?? body.requested_modality_id, requestedExamTypeId: body.requestedExamTypeId ?? body.requested_exam_type_id, originalReportDependency: body.originalReportDependency ?? body.original_report_dependency, notifyOnArrival: body.notifyOnArrival ?? body.notify_on_arrival, notifyOnImagingCompleted: body.notifyOnImagingCompleted ?? body.notify_on_imaging_completed, requestedByUserId: userId! }));
...
router.patch(
  ...
  asyncRoute(async (req: DoctorRequest, res: Response) => {
    ...
    const result = await rescheduleBooking(
      ...
      true,
      false,
      async (client, rescheduled) => {
        if (!(await canDoctorProtocolModality(userId, Number(rescheduled.booking.modalityId), client))) {
          throw new HttpError(403, "Doctor is not permitted to protocol this appointment modality.");
        }
        const examTypeIds = [rescheduled.previousExamTypeId, rescheduled.booking.examTypeId]
          .filter((id): id is number => id != null);
        const examTypes = await client.query<{ id: number; name: string | null }>(
          `select id, coalesce(nullif(name_en, ''), nullif(name_ar, '')) as name
           from exam_types
           where id = any($1::bigint[])`,
          [examTypeIds]
        );
        const examTypeNames = new Map(examTypes.rows.map((row) => [Number(row.id), row.name]));
        await logAuditEntry({
          entityType: "appointment",
          entityId: appointmentId,
          actionType: "doctor_protocoling_exam_type_changed",
          oldValues: {
            examTypeId: rescheduled.previousExamTypeId,
            examTypeName: rescheduled.previousExamTypeId == null ? null : examTypeNames.get(rescheduled.previousExamTypeId) ?? null,
          },
          newValues: {
            examTypeId,
            examTypeName: examTypeNames.get(examTypeId) ?? null,
            source: "Doctor Protocoling Board",
          },
