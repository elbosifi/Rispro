...
describe("full protocol XLSX import", () => {
  it("exports canonical protocol rows with deterministic workbook keys and preserved order", async () => {
    const detail = {
      protocol: { id: 42, name: "CT Brain Routine", modality: "CT", anatomyRegionName: "Brain", category: "General", indication: "Headache", contrastPolicy: "Non-contrast", oralContrastPolicy: null, bowelPreparation: null, preparationNotes: null },
    ...
    assert.deepEqual(workbook.SheetNames, ["Protocols", "CT Phases", "CT Techniques", "MRI Sequences", "Instructions"]);
  ...
  it("exports a valid empty canonical workbook when the protocol library is empty", async () => {
    ...
    try {
      ...
      assert.deepEqual(parsed.SheetNames, ["Protocols", "CT Phases", "CT Techniques", "MRI Sequences", "Instructions"]);
