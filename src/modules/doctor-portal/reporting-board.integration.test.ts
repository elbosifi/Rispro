...
function installDefaultSonicDicomReadersForTest(): void {
  if (!sonicDicomCacheService) return;
...
describe("Reporting Assignment Board DB-backed integration", { skip: skipEnv }, () => {
  ...
  it("protects a primary Final from a newer comparison Draft during single primary refresh", async () => {
    guard();
    const date = addDays(104);
    const label = uniq("primary_refresh_comparison");
    const comparisonDoctor = await createDoctorUser("primary_refresh_comparison_doctor", "doctor", { canReportModalities: [ctModalityId, mrModalityId] });
    const comparisonEmail = `${uniq("primary_refresh_comparison_email").toLowerCase()}@nccb.ly`;
    ...
    await pool.query(`update doctor_portal.reporting_board_sonicdicom_cache set report_status = 'final', report_final_at = $2::timestamptz, sonicdicom_latest_document_id = 'A', source = 'sonicdicom', last_success_at = now(), last_attempt_at = now(), next_check_at = now() + interval '1 hour' where appointment_id = $1`, [source, `${date}T09:00:00.000Z`]);
    await pool.query(`insert into doctor_portal.comparison_sonicdicom_cache (comparison_assignment_id, comparison_request_id, report_status, last_success_at, last_attempt_at, next_check_at) values ($1, $2, 'draft', now(), now(), now() + interval '2 hours')`, [assignmentId, comparison]);

    const dependency = (await sonicDicomCacheService.selectComparisonSonicDicomCacheCandidatesByAppointmentIds([source, source, 0, -1])).find((candidate) => candidate.comparisonAssignmentId === assignmentId);
    assert.equal(dependency?.assignedDoctorSonicAccount, comparisonEmail);
    assert.equal(dependency?.storedDocumentId, null);
    assert.equal((await sonicDicomCacheService.selectDueComparisonSonicDicomCacheCandidates(200)).some((candidate) => candidate.comparisonAssignmentId === assignmentId), false);

    let rawGenericStatus: { state: string; latestDocumentId: string | null } | null = null;
    sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest({
      checkStatusesBatch: async (contexts) => new Map(contexts.map((context) => {
        rawGenericStatus = { state: "draft", latestDocumentId: "B" };
        return [context.bookingId, { state: "draft" as const, canViewReport: false, source: "sonicdicom" as const, reportFinalAt: null, latestDocumentId: "B", finalizedByAccount: null, correlationMethod: "study_instance_uid" as const }];
      })),
    ...
    try {
      const response = await api<{ ok: boolean; appointmentId: number; successful: boolean; previousStatus: string; reportStatus: string; changed: boolean; cachedStatusRetained: boolean; checkedAt: string }>(supervisor.cookie, `/api/doctor/reporting-board/cases/${source}/refresh-sonicdicom`, { method: "POST" });
      assert.equal(response.status, 200, JSON.stringify(response.data));
      assert.deepEqual({ ok: response.data.ok, appointmentId: response.data.appointmentId, successful: response.data.successful, previousStatus: response.data.previousStatus, reportStatus: response.data.reportStatus, changed: response.data.changed, cachedStatusRetained: response.data.cachedStatusRetained }, { ok: true, appointmentId: source, successful: true, previousStatus: "final", reportStatus: "final", changed: false, cachedStatusRetained: false });
      assert.deepEqual(rawGenericStatus, { state: "draft", latestDocumentId: "B" });

      const primaryCache = await pool.query<{ report_status: string; sonicdicom_latest_document_id: string | null }>(`select report_status, sonicdicom_latest_document_id from doctor_portal.reporting_board_sonicdicom_cache where appointment_id = $1`, [source]);
      assert.deepEqual(primaryCache.rows[0], { report_status: "final", sonicdicom_latest_document_id: "A" });
      const comparisonCache = await pool.query<{ report_status: string; sonicdicom_document_id: string | null; sonicdicom_account: string | null }>(`select report_status, sonicdicom_document_id, sonicdicom_account from doctor_portal.comparison_sonicdicom_cache where comparison_assignment_id = $1`, [assignmentId]);
      assert.deepEqual(comparisonCache.rows[0], { report_status: "draft", sonicdicom_document_id: "B", sonicdicom_account: comparisonEmail });
      const comparisonDocument = await pool.query<{ sonicdicom_document_id: string; sonicdicom_account: string | null }>(`select sonicdicom_document_id, sonicdicom_account from doctor_portal.comparison_sonicdicom_documents where comparison_assignment_id = $1 and sonicdicom_document_id = 'B'`, [assignmentId]);
      assert.deepEqual(comparisonDocument.rows[0], { sonicdicom_document_id: "B", sonicdicom_account: comparisonEmail });
    } finally {
      sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest(null);
    }
  });

  it("protects a hidden comparison during board refresh filtered to appointments", async () => {
    guard();
    const date = addDays(105);
    const label = uniq("board_refresh_hidden_comparison");
    const comparisonDoctor = await createDoctorUser("board_refresh_hidden_comparison_doctor", "doctor", { canReportModalities: [ctModalityId, mrModalityId] });
    const comparisonEmail = `${uniq("board_refresh_hidden_comparison_email").toLowerCase()}@nccb.ly`;
    await pool.query(`update users set email = $1 where id = $2`, [comparisonEmail, comparisonDoctor.id]);
    const source = await createBooking({ modalityId: ctModalityId, examTypeId: ctExamTypeId, date, patientName: label });
    const comparison = await createComparisonRequestForBooking(source, `${date}T10:00:00.000Z`, label);
    await assignComparisonDirectly(comparison, comparisonDoctor.doctorId);
    ...
    const board = await api<{ cases: Array<{ caseType: string; appointmentId: number; comparisonRequestId: number | null }> }>(supervisor.cookie, `/api/doctor/reporting-board/cases?dateFrom=${date}&dateTo=${date}&caseSource=appointments&reportStatus=all&limit=20`);
    assert.equal(board.status, 200, JSON.stringify(board.data));
    assert.equal(board.data.cases.some((row) => row.appointmentId === source && row.caseType === "appointment"), true);
    assert.equal(board.data.cases.some((row) => row.comparisonRequestId === comparison), false);
    ...
    try {
      const response = await api<{ ok: boolean; checked: number; successful: number; failed: number; checkedAt: string }>(supervisor.cookie, "/api/doctor/reporting-board/refresh-sonicdicom", {
        method: "POST",
        body: { filters: { dateFrom: date, dateTo: date, caseSource: "appointments", reportStatus: "all" } },
      });
      assert.equal(response.status, 200, JSON.stringify(response.data));
      assert.deepEqual({ ok: response.data.ok, checked: response.data.checked, successful: response.data.successful, failed: response.data.failed }, { ok: true, checked: 1, successful: 1, failed: 0 });
  ...
  it("refreshes a single comparison through SonicDICOM document history without changing the primary", async () => {
    guard();
    const date = addDays(100);
    const label = uniq("comparison_manual_refresh");
    const comparisonDoctor = await createDoctorUser("comparison_email_identity", "doctor", { canReportModalities: [ctModalityId, mrModalityId] });
    const username = (await pool.query<{ username: string }>(`select username from users where id = $1`, [comparisonDoctor.id])).rows[0].username;
    const email = `${uniq("comparison_email").toLowerCase()}@nccb.ly`;
    assert.notEqual(username, email);
    await pool.query(`update users set email = $1 where id = $2`, [email, comparisonDoctor.id]);
    const source = await createBooking({ modalityId: ctModalityId, examTypeId: ctExamTypeId, date, patientName: label });
    const comparison = await createComparisonRequestForBooking(source, `${date}T09:00:00.000Z`, label);
    await assignComparisonDirectly(comparison, comparisonDoctor.doctorId);
    const assignmentId = Number((await pool.query<{ id: string }>(`select id::text from doctor_portal.comparison_case_assignments where comparison_request_id = $1 and status = 'active'`, [comparison])).rows[0].id);
    ...
    const dueCandidate = (await sonicDicomCacheService.selectDueComparisonSonicDicomCacheCandidates(20)).find((candidate) => candidate.comparisonAssignmentId === assignmentId);
    const manualCandidate = (await sonicDicomCacheService.selectComparisonSonicDicomCacheCandidatesByRequestIds([comparison])).find((candidate) => candidate.comparisonAssignmentId === assignmentId);
    assert.equal(dueCandidate?.assignedDoctorSonicAccount, email);
    assert.equal(manualCandidate?.assignedDoctorSonicAccount, email);
    await seedSonicDicomCache(source, "final");
    await pool.query(`update doctor_portal.reporting_board_sonicdicom_cache set sonicdicom_latest_document_id = 'A', report_status = 'final' where appointment_id = $1`, [source]);
    sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest({
      checkStatusesBatch: async (contexts) => new Map(contexts.map((context) => [context.bookingId, { state: "final" as const, canViewReport: true, source: "sonicdicom" as const, reportFinalAt: `${date}T11:00:00.000Z`, latestDocumentId: "A", finalizedByAccount: "primary@nccb.ly", correlationMethod: "accession_fallback" as const }])),
      fetchDocumentHistoriesBatch: async (contexts) => new Map(contexts.map((context) => [context.lookupKey, {
        foundStudy: true,
        foundReport: true,
        reportNo: 9284,
        ...
        documents: [
          { reportNo: 9284, documentId: "B", account: email, statusCode: 1, updatedAt: `${date}T11:00:00.000Z` },
          { reportNo: 9284, documentId: "A", account: "primary@nccb.ly", statusCode: 6, updatedAt: `${date}T09:00:00.000Z` },
        ],
      }])),
    });
    try {
      const response = await api<{ ok: boolean; comparisonRequestId: number; successful: boolean; reportStatus: string; cachedStatusRetained: boolean; checkedAt: string }>(supervisor.cookie, `/api/doctor/reporting-board/comparisons/${comparison}/refresh-sonicdicom`, { method: "POST" });
      assert.equal(response.status, 200, JSON.stringify(response.data));
      assert.deepEqual({ ok: response.data.ok, comparisonRequestId: response.data.comparisonRequestId, successful: response.data.successful, reportStatus: response.data.reportStatus, cachedStatusRetained: response.data.cachedStatusRetained }, { ok: true, comparisonRequestId: comparison, successful: true, reportStatus: "draft", cachedStatusRetained: false });

      const comparisonCache = await pool.query<{ report_status: string; sonicdicom_document_id: string | null; sonicdicom_account: string | null }>(`select report_status, sonicdicom_document_id, sonicdicom_account from doctor_portal.comparison_sonicdicom_cache where comparison_assignment_id = $1`, [assignmentId]);
      assert.deepEqual(comparisonCache.rows[0], { report_status: "draft", sonicdicom_document_id: "B", sonicdicom_account: email });
      const comparisonDocument = await pool.query<{ sonicdicom_account: string | null }>(`select sonicdicom_account from doctor_portal.comparison_sonicdicom_documents where comparison_assignment_id = $1 and sonicdicom_document_id = 'B'`, [assignmentId]);
      assert.deepEqual(comparisonDocument.rows[0], { sonicdicom_account: email });
      const primaryCache = await pool.query<{ report_status: string; sonicdicom_latest_document_id: string | null }>(`select report_status, sonicdicom_latest_document_id from doctor_portal.reporting_board_sonicdicom_cache where appointment_id = $1`, [source]);
      assert.deepEqual(primaryCache.rows[0], { report_status: "final", sonicdicom_latest_document_id: "A" });
    } finally {
      sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest(null);
  ...
  it("retains legacy username correlation when the assigned user has no email", async () => {
    guard();
    const date = addDays(103);
    const label = uniq("comparison_legacy_identity");
    const legacyDoctor = await createDoctorUser("comparison_legacy_identity", "doctor", { canReportModalities: [ctModalityId, mrModalityId] });
    const username = (await pool.query<{ username: string }>(`select username from users where id = $1`, [legacyDoctor.id])).rows[0].username;
    await pool.query(`update users set email = null where id = $1`, [legacyDoctor.id]);
    const source = await createBooking({ modalityId: ctModalityId, examTypeId: ctExamTypeId, date, patientName: label });
    const comparison = await createComparisonRequestForBooking(source, `${date}T09:00:00.000Z`, label);
    await assignComparisonDirectly(comparison, legacyDoctor.doctorId);
    const assignmentId = Number((await pool.query<{ id: string }>(`select id::text from doctor_portal.comparison_case_assignments where comparison_request_id = $1 and status = 'active'`, [comparison])).rows[0].id);
    await pool.query(`update doctor_portal.comparison_case_assignments set assigned_at = $2::timestamptz where id = $1`, [assignmentId, `${date}T10:00:00.000Z`]);
    const dueCandidate = (await sonicDicomCacheService.selectDueComparisonSonicDicomCacheCandidates(20)).find((candidate) => candidate.comparisonAssignmentId === assignmentId);
    const manualCandidate = (await sonicDicomCacheService.selectComparisonSonicDicomCacheCandidatesByRequestIds([comparison])).find((candidate) => candidate.comparisonAssignmentId === assignmentId);
    assert.equal(dueCandidate?.assignedDoctorSonicAccount, username);
    assert.equal(manualCandidate?.assignedDoctorSonicAccount, username);
    await seedSonicDicomCache(source, "final");
    await pool.query(`update doctor_portal.reporting_board_sonicdicom_cache set sonicdicom_latest_document_id = 'A', report_status = 'final' where appointment_id = $1`, [source]);
    sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest({
      checkStatusesBatch: async (contexts) => new Map(contexts.map((context) => [context.bookingId, { state: "final" as const, canViewReport: true, source: "sonicdicom" as const, reportFinalAt: `${date}T11:00:00.000Z`, latestDocumentId: "A", finalizedByAccount: "primary@nccb.ly", correlationMethod: "accession_fallback" as const }])),
    ...
    try {
      const response = await api<{ successful: boolean; reportStatus: string }>(supervisor.cookie, `/api/doctor/reporting-board/comparisons/${comparison}/refresh-sonicdicom`, { method: "POST" });
      assert.equal(response.status, 200, JSON.stringify(response.data));
      assert.deepEqual({ successful: response.data.successful, reportStatus: response.data.reportStatus }, { successful: true, reportStatus: "draft" });
      const comparisonCache = await pool.query<{ report_status: string; sonicdicom_document_id: string | null; sonicdicom_account: string | null }>(`select report_status, sonicdicom_document_id, sonicdicom_account from doctor_portal.comparison_sonicdicom_cache where comparison_assignment_id = $1`, [assignmentId]);
      assert.deepEqual(comparisonCache.rows[0], { report_status: "draft", sonicdicom_document_id: "B", sonicdicom_account: username });
      const primaryCache = await pool.query<{ report_status: string; sonicdicom_latest_document_id: string | null }>(`select report_status, sonicdicom_latest_document_id from doctor_portal.reporting_board_sonicdicom_cache where appointment_id = $1`, [source]);
      assert.deepEqual(primaryCache.rows[0], { report_status: "final", sonicdicom_latest_document_id: "A" });
    } finally {
      sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest(null);
  ...
  it("retains comparison and primary safe caches when a manual comparison refresh fails", async () => {
    guard();
    const date = addDays(101);
    const label = uniq("comparison_manual_failure");
    const source = await createBooking({ modalityId: ctModalityId, examTypeId: ctExamTypeId, date, patientName: label });
    const comparison = await createComparisonRequestForBooking(source, `${date}T09:00:00.000Z`, label);
    ...
    await pool.query(`
      insert into doctor_portal.comparison_sonicdicom_cache (
        comparison_assignment_id, comparison_request_id, report_status, sonicdicom_document_id,
        sonicdicom_document_updated_at, last_success_at, last_attempt_at, next_check_at
      ) values ($1, $2, 'draft', 'B', $3::timestamptz, now(), now(), now())
    `, [assignmentId, comparison, `${date}T11:00:00.000Z`]);
    sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest({
      checkStatusesBatch: async (contexts) => new Map(contexts.map((context) => [context.bookingId, { state: "final" as const, canViewReport: true, source: "sonicdicom" as const, reportFinalAt: `${date}T11:00:00.000Z`, latestDocumentId: "A", finalizedByAccount: "primary@nccb.ly", correlationMethod: "accession_fallback" as const }])),
      fetchDocumentHistoriesBatch: async () => { throw new Error("history unavailable"); },
    });
    try {
      const response = await api<{ successful: boolean; reportStatus: string; cachedStatusRetained: boolean }>(supervisor.cookie, `/api/doctor/reporting-board/comparisons/${comparison}/refresh-sonicdicom`, { method: "POST" });
      assert.equal(response.status, 200, JSON.stringify(response.data));
      assert.deepEqual({ successful: response.data.successful, reportStatus: response.data.reportStatus, cachedStatusRetained: response.data.cachedStatusRetained }, { successful: false, reportStatus: "draft", cachedStatusRetained: true });

      const comparisonCache = await pool.query<{ report_status: string; sonicdicom_document_id: string | null }>(`select report_status, sonicdicom_document_id from doctor_portal.comparison_sonicdicom_cache where comparison_assignment_id = $1`, [assignmentId]);
      assert.deepEqual(comparisonCache.rows[0], { report_status: "draft", sonicdicom_document_id: "B" });
      const primaryCache = await pool.query<{ report_status: string; sonicdicom_latest_document_id: string | null }>(`select report_status, sonicdicom_latest_document_id from doctor_portal.reporting_board_sonicdicom_cache where appointment_id = $1`, [source]);
      assert.deepEqual(primaryCache.rows[0], { report_status: "final", sonicdicom_latest_document_id: "A" });
    } finally {
      sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest(null);
  ...
  it("includes filtered comparison rows in the board-level SonicDICOM refresh counts", async () => {
    guard();
    const date = addDays(102);
    const label = uniq("comparison_board_refresh");
    const source = await createBooking({ modalityId: ctModalityId, examTypeId: ctExamTypeId, date, patientName: label });
    const comparison = await createComparisonRequestForBooking(source, `${date}T09:00:00.000Z`, label);
    await assignComparisonDirectly(comparison, targetDoctor.doctorId);
    const assignmentId = Number((await pool.query<{ id: string }>(`select id::text from doctor_portal.comparison_case_assignments where comparison_request_id = $1 and status = 'active'`, [comparison])).rows[0].id);
    ...
    sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest({
      ...
      fetchDocumentHistoriesBatch: async (contexts) => new Map(contexts.map((context) => [context.lookupKey, {
        ...
        reportNo: 9284,
        correlationMethod: "accession_fallback" as const,
        documents: [{ reportNo: 9284, documentId: "B", account: username, statusCode: 1, updatedAt: `${date}T11:00:00.000Z` }],
      }])),
    });
    try {
      const response = await api<{ checked: number; successful: number; failed: number }>(supervisor.cookie, "/api/doctor/reporting-board/refresh-sonicdicom", {
        method: "POST",
        body: { filters: { dateFrom: date, dateTo: date, caseSource: "comparisons", reportStatus: "all" } },
      });
      assert.equal(response.status, 200, JSON.stringify(response.data));
      assert.deepEqual({ checked: response.data.checked, successful: response.data.successful, failed: response.data.failed }, { checked: 1, successful: 1, failed: 0 });
    } finally {
      sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest(null);
    }
  });
  ...
  it("self-heals a polluted primary after confidently correlating comparison B", async () => {
    guard();
    const date = addDays(89);
    const label = uniq("comparison_bootstrap_self_heal");
    const source = await createBooking({ modalityId: ctModalityId, examTypeId: ctExamTypeId, date, patientName: label });
    const comparison = await createComparisonRequestForBooking(source, `${date}T10:00:00.000Z`, label);
    await assignComparisonDirectly(comparison, targetDoctor.doctorId);
    const assignmentId = Number((await pool.query<{ id: string }>(`select id::text from doctor_portal.comparison_case_assignments where comparison_request_id = $1 and status = 'active'`, [comparison])).rows[0].id);
    ...
    try {
      const tick = await sonicDicomCacheService.refreshReportingBoardSonicDicomCacheCandidates([primary], [comparisonCandidate]);
      assert.deepEqual({ successful: tick.successful, failed: tick.failed }, { successful: 2, failed: 0 });
      const primaryRow = (await pool.query<{ report_status: string; sonicdicom_latest_document_id: string | null }>(`select report_status, sonicdicom_latest_document_id from doctor_portal.reporting_board_sonicdicom_cache where appointment_id = $1`, [source])).rows[0];
      assert.deepEqual(primaryRow, { report_status: "final", sonicdicom_latest_document_id: "A" });
      const comparisonRow = (await pool.query<{ report_status: string; sonicdicom_document_id: string | null }>(`select report_status, sonicdicom_document_id from doctor_portal.comparison_sonicdicom_cache where comparison_assignment_id = $1`, [assignmentId])).rows[0];
      assert.deepEqual(comparisonRow, { report_status: "draft", sonicdicom_document_id: "B" });
      assert.equal((await pool.query(`select count(*)::int as count from doctor_portal.comparison_sonicdicom_documents where comparison_assignment_id = $1 and sonicdicom_document_id = 'B'`, [assignmentId])).rows[0].count, 1);
    } finally {
      sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest(null);
    }
  });

  it("fails closed for an ambiguous polluted primary bootstrap", async () => {
    guard();
    const date = addDays(90);
    const label = uniq("comparison_bootstrap_ambiguous");
    const source = await createBooking({ modalityId: ctModalityId, examTypeId: ctExamTypeId, date, patientName: label });
    const comparison = await createComparisonRequestForBooking(source, `${date}T10:00:00.000Z`, label);
    await assignComparisonDirectly(comparison, targetDoctor.doctorId);
    const assignmentId = Number((await pool.query<{ id: string }>(`select id::text from doctor_portal.comparison_case_assignments where comparison_request_id = $1 and status = 'active'`, [comparison])).rows[0].id);
    ...
    try {
      await sonicDicomCacheService.refreshReportingBoardSonicDicomCacheCandidates([primary], [comparisonCandidate]);
      const primaryRow = (await pool.query<{ report_status: string; sonicdicom_latest_document_id: string | null }>(`select report_status, sonicdicom_latest_document_id from doctor_portal.reporting_board_sonicdicom_cache where appointment_id = $1`, [source])).rows[0];
      assert.deepEqual(primaryRow, { report_status: "draft", sonicdicom_latest_document_id: "B" });
      const comparisonRow = (await pool.query<{ report_status: string; sonicdicom_document_id: string | null }>(`select report_status, sonicdicom_document_id from doctor_portal.comparison_sonicdicom_cache where comparison_assignment_id = $1`, [assignmentId])).rows[0];
      assert.deepEqual(comparisonRow, { report_status: "no_report", sonicdicom_document_id: null });
      assert.equal((await pool.query(`select count(*)::int as count from doctor_portal.comparison_sonicdicom_documents where comparison_assignment_id = $1 and sonicdicom_document_id = 'B'`, [assignmentId])).rows[0].count, 0);
    } finally {
      sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest(null);
    }
  });

  it("uses the cached B timestamp and retries safely when B is physically missing", async () => {
    guard();
    const date = addDays(91);
    const label = uniq("comparison_missing_stored");
    const source = await createBooking({ modalityId: ctModalityId, examTypeId: ctExamTypeId, date, patientName: label });
    const comparison = await createComparisonRequestForBooking(source, `${date}T10:00:00.000Z`, label);
    await assignComparisonDirectly(comparison, targetDoctor.doctorId);
    const assignmentId = Number((await pool.query<{ id: string }>(`select id::text from doctor_portal.comparison_case_assignments where comparison_request_id = $1 and status = 'active'`, [comparison])).rows[0].id);
    const username = (await pool.query<{ username: string }>(`select username from users where id = $1`, [targetDoctor.id])).rows[0].username;
    const storedUpdatedAt = `${date}T11:00:00.000Z`;
    await pool.query(`update doctor_portal.comparison_case_assignments set assigned_at = $2::timestamptz where id = $1`, [assignmentId, `${date}T10:00:00.000Z`]);
    await pool.query(`update doctor_portal.reporting_board_sonicdicom_cache set report_status = 'draft', sonicdicom_latest_document_id = 'A', last_success_at = now(), next_check_at = now() where appointment_id = $1`, [source]);
    await pool.query(`insert into doctor_portal.comparison_sonicdicom_cache (comparison_assignment_id, comparison_request_id, report_status, sonicdicom_document_id, sonicdicom_document_updated_at, last_success_at, last_attempt_at, next_check_at) values ($1, $2, 'draft', 'B', $3::timestamptz, now(), now(), now())`, [assignmentId, comparison, storedUpdatedAt]);
    await pool.query(`insert into doctor_portal.comparison_sonicdicom_documents (comparison_assignment_id, sonicdicom_document_id, document_updated_at, first_seen_at, last_seen_at) values ($1, 'B', $2::timestamptz, now(), now())`, [assignmentId, storedUpdatedAt]);
    ...
    try {
      await sonicDicomCacheService.refreshReportingBoardSonicDicomCacheCandidates([primary], [due]);
  ...
  it("does not let a tombstoned non-comparison document mask the Final primary", async () => {
    guard();
    const date = addDays(92);
    const label = uniq("tombstoned_non_comparison_primary");
    const source = await createBooking({ modalityId: ctModalityId, examTypeId: ctExamTypeId, date, patientName: label });
    const comparison = await createComparisonRequestForBooking(source, `${date}T10:00:00.000Z`, label);
    await assignComparisonDirectly(comparison, targetDoctor.doctorId);
    const assignmentId = Number((await pool.query<{ id: string }>(`select id::text from doctor_portal.comparison_case_assignments where comparison_request_id = $1 and status = 'active'`, [comparison])).rows[0].id);
  ...
  it("retains a cached primary Final when document history is unavailable", async () => {
    ...
    try {
      const tick = await sonicDicomCacheService.refreshReportingBoardSonicDicomCacheCandidates([primary]);
      assert.deepEqual({ successful: tick.successful, failed: tick.failed }, { successful: 0, failed: 1 });
  ...
  it("revalidates automatic assignment through the protected primary when comparison Draft is newer", async () => {
    guard();
    const date = addDays(112);
    const label = uniq("assignment_primary_comparison");
    const comparisonDoctor = await createDoctorUser("assignment_primary_comparison_doctor", "doctor", { canReportModalities: [ctModalityId, mrModalityId] });
    const comparisonEmail = `${uniq("assignment_primary_comparison_email").toLowerCase()}@nccb.ly`;
    await pool.query(`update users set email = $1 where id = $2`, [comparisonEmail, comparisonDoctor.id]);
    const primary = await createBooking({
      modalityId: ctModalityId,
      examTypeId: ctExamTypeId,
      priorityId: statPriorityId,
      date,
      completedAt: "2026-08-30T08:00:00.000Z",
      patientName: label,
    });
    const fallback = await createBooking({
      modalityId: ctModalityId,
      examTypeId: ctExamTypeId,
      priorityId: routinePriorityId,
      date,
      completedAt: "2026-08-30T09:00:00.000Z",
      patientName: `${label} fallback`,
    });
    const comparison = await createComparisonRequestForBooking(primary, `${date}T10:00:00.000Z`, label);
    await assignComparisonDirectly(comparison, comparisonDoctor.doctorId);
    const assignmentId = Number((await pool.query<{ id: string }>(`select id::text from doctor_portal.comparison_case_assignments where comparison_request_id = $1 and status = 'active'`, [comparison])).rows[0].id);
    await pool.query(`update doctor_portal.comparison_case_assignments set assigned_at = $2::timestamptz where id = $1`, [assignmentId, `${date}T10:30:00.000Z`]);
    ...
    const visiblePrimary = visible.data.cases.find((row) => row.appointmentId === primary);
    assert.deepEqual(visiblePrimary && { reportStatus: visiblePrimary.reportStatus, canAssign: visiblePrimary.canAssign }, { reportStatus: "draft", canAssign: true });
    assert.equal((await sonicDicomCacheService.selectDueComparisonSonicDicomCacheCandidates(200)).some((candidate) => candidate.comparisonAssignmentId === assignmentId), false);

    let rawGenericStatus: { state: string; latestDocumentId: string | null } | null = null;
    sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest({
      checkStatusesBatch: async (contexts) => new Map(contexts.map((context) => {
        const primaryResult = context.bookingId === primary;
        if (primaryResult) rawGenericStatus = { state: "draft", latestDocumentId: "B" };
        return [context.bookingId, {
          state: "draft" as const,
          canViewReport: false,
          source: "sonicdicom" as const,
          reportFinalAt: null,
          latestDocumentId: primaryResult ? "B" : null,
    ...
    try {
      const response = await api<{ assignedCount: number; assignedAppointmentIds: number[] }>(
        supervisor.cookie,
        "/api/doctor/reporting-board/bulk-assign-next",
        { method: "POST", body: { doctorId: targetDoctor.doctorId, count: 1, filters: { dateFrom: date, dateTo: date, reportStatus: "all" }, reason: "comparison-safe assignment revalidation" } }
      );
      assert.equal(response.status, 200, JSON.stringify(response.data));
      assert.deepEqual(response.data.assignedAppointmentIds, [fallback]);
      assert.equal(response.data.assignedCount, 1);
      assert.deepEqual(rawGenericStatus, { state: "draft", latestDocumentId: "B" });
      ...
      assert.equal((await pool.query(`select 1 from doctor_portal.comparison_sonicdicom_documents where comparison_assignment_id = $1 and sonicdicom_document_id = 'B'`, [assignmentId])).rowCount, 1);
      assert.equal((await pool.query(`select 1 from doctor_portal.case_team_assignments where appointment_id = $1 and assignment_type = 'reporting' and status = 'active'`, [primary])).rowCount, 0);
      assert.equal((await pool.query(`select 1 from doctor_portal.case_team_assignments where appointment_id = $1 and assigned_doctor_id = $2 and assignment_type = 'reporting' and status = 'active'`, [fallback, targetDoctor.doctorId])).rowCount, 1);
    } finally {
      sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest(null);
    }
  });

  it("fails closed when protected comparison history is unavailable and primary Draft is retained", async () => {
    guard();
    const date = addDays(113);
    const label = uniq("assignment_history_failure");
    const primary = await createBooking({
      modalityId: ctModalityId,
      examTypeId: ctExamTypeId,
      priorityId: statPriorityId,
    ...
    await assignComparisonDirectly(comparison, targetDoctor.doctorId);
    const assignmentId = Number((await pool.query<{ id: string }>(`select id::text from doctor_portal.comparison_case_assignments where comparison_request_id = $1 and status = 'active'`, [comparison])).rows[0].id);
    const comparisonAccount = (await pool.query<{ username: string }>(`select username from users where id = $1`, [targetDoctor.id])).rows[0].username;
    await pool.query(`update doctor_portal.comparison_case_assignments set assigned_at = $2::timestamptz where id = $1`, [assignmentId, `${date}T10:30:00.000Z`]);
    await seedSonicDicomCache(primary, "draft");
    await seedSonicDicomCache(fallback, "draft");
    await pool.query(`update doctor_portal.reporting_board_sonicdicom_cache set report_status = 'draft', sonicdicom_latest_document_id = 'B', last_success_at = now() - interval '5 minutes', last_attempt_at = now() - interval '5 minutes', next_check_at = now() + interval '2 hours' where appointment_id = $1`, [primary]);
    await pool.query(`insert into doctor_portal.comparison_sonicdicom_cache (comparison_assignment_id, comparison_request_id, report_status, sonicdicom_report_no, sonicdicom_document_id, sonicdicom_account, last_success_at, last_attempt_at, next_check_at) values ($1, $2, 'draft', 9284, 'B', $3, now() - interval '5 minutes', now() - interval '5 minutes', now() + interval '2 hours')`, [assignmentId, comparison, comparisonAccount]);
    await pool.query(`insert into doctor_portal.comparison_sonicdicom_documents (comparison_assignment_id, sonicdicom_document_id, document_updated_at, first_seen_at, last_seen_at) values ($1, 'B', $2::timestamptz, now(), now())`, [assignmentId, `${date}T11:00:00.000Z`]);
    const before = (await pool.query<{ report_status: string; sonicdicom_latest_document_id: string | null; last_attempt_at: Date; last_success_at: Date }>(`select report_status, sonicdicom_latest_document_id, last_attempt_at, last_success_at from doctor_portal.reporting_board_sonicdicom_cache where appointment_id = $1`, [primary])).rows[0];
    ...
    sonicDicomCacheService.__setReportingBoardSonicDicomReadersForTest({
      ...
      fetchDocumentHistoriesBatch: async () => { throw new Error("history unavailable during assignment revalidation"); },
    });
    try {
      const response = await api<{ assignedCount: number; assignedAppointmentIds: number[] }>(
        supervisor.cookie,
        "/api/doctor/reporting-board/bulk-assign-next",
        { method: "POST", body: { doctorId: targetDoctor.doctorId, count: 1, filters: { dateFrom: date, dateTo: date, reportStatus: "all" }, reason: "failed comparison revalidation" } }
      );
      assert.equal(response.status, 200, JSON.stringify(response.data));
      assert.deepEqual(response.data.assignedAppointmentIds, [fallback]);
      assert.equal(response.data.assignedCount, 1);
      ...
      assert.deepEqual({ report_status: after.report_status, sonicdicom_latest_document_id: after.sonicdicom_latest_document_id }, { report_status: "draft", sonicdicom_latest_document_id: "B" });
      assert.ok(after.last_attempt_at.getTime() > before.last_attempt_at.getTime());
