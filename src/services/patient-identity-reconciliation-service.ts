...
const idsEqual = (a: string[] | null, b: string[]) => equal([...(a || [])].sort(), [...b].sort());
const idsContained=(required:string[]|null,actual:string[])=>new Set(actual).size>=new Set(required||[]).size&&(required||[]).every((value)=>actual.includes(value));
...
async function patient(job: PatientIdentityReconciliationJob) { const row = (await pool.query<{ identifier_value:string|null; national_id:string|null; mrn:string|null; english_full_name:string|null; arabic_full_name:string|null; estimated_date_of_birth:string|null }>("select identifier_value,national_id,mrn,english_full_name,arabic_full_name,estimated_date_of_birth::text from patients where id=$1", [job.patient_id])).rows[0]; if (!row) throw new HttpError(404,"Patient not found."); const patientId=clean(row.identifier_value || row.national_id || row.mrn); if(!patientId) throw new HttpError(409,"The selected RISpro patient has no primary Patient ID."); return { patientId, name: clean(row.english_full_name || row.arabic_full_name) || "RISpro Patient", birthDate: row.estimated_date_of_birth }; }
...
function verify(before: OrthancPatientIdentitySnapshot, after: OrthancPatientIdentitySnapshot, patientId: string, expectedSequence: Array<Record<string,unknown>>) {
  if (after.patientId !== patientId || !equal(after.otherPatientIdsSequence, expectedSequence) || before.patientName !== after.patientName || before.patientBirthDate !== after.patientBirthDate || before.patientSex !== after.patientSex || before.studyInstanceUid !== after.studyInstanceUid || before.accessionNumber !== after.accessionNumber || !idsEqual(before.seriesInstanceUids,after.seriesInstanceUids) || !idsEqual(before.sopInstanceUids,after.sopInstanceUids)) throw new HttpError(502,"Patient Identity Reconciliation verification failed.",{code:"PATIENT_IDENTITY_RECONCILIATION_VERIFICATION_FAILED"});
}
...
function persistedSnapshot(job:PatientIdentityReconciliationJob):OrthancPatientIdentitySnapshot|null {
  if(!job.orthanc_study_id_before||job.original_other_patient_ids==null||job.original_series_instance_uids==null||job.original_sop_instance_uids==null)return null;
  return {orthancStudyId:job.orthanc_study_id_before,studyInstanceUid:job.study_instance_uid,accessionNumber:job.accession_number,patientId:job.old_patient_id,patientName:job.original_patient_name,patientBirthDate:job.original_patient_birth_date,patientSex:job.original_patient_sex,studyDate:job.study_date,studyDescription:null,modalitiesInStudy:[],seriesCount:job.original_series_instance_uids.length,instanceCount:job.original_sop_instance_uids.length,otherPatientIdsSequence:job.original_other_patient_ids,seriesInstanceUids:job.original_series_instance_uids,sopInstanceUids:job.original_sop_instance_uids};
}
function verifyRecoveredTarget(before:OrthancPatientIdentitySnapshot,after:OrthancPatientIdentitySnapshot,patientId:string,sequence:Array<Record<string,unknown>>,jobId:number,auditExists:boolean){
  if(after.patientId!==patientId||!equal(after.otherPatientIdsSequence,sequence)||before.patientName!==after.patientName||before.patientBirthDate!==after.patientBirthDate||before.patientSex!==after.patientSex||before.studyInstanceUid!==after.studyInstanceUid||before.accessionNumber!==after.accessionNumber)throw new HttpError(502,"Patient Identity Reconciliation verification failed.",{code:"PATIENT_IDENTITY_RECONCILIATION_VERIFICATION_FAILED"});
  const expectedSeries=auditExists?[...before.seriesInstanceUids,auditUid(jobId,"series")]:before.seriesInstanceUids;
  const expectedSops=auditExists?[...before.sopInstanceUids,auditUid(jobId,"instance")]:before.sopInstanceUids;
  if(!idsEqual(expectedSeries,after.seriesInstanceUids)||!idsEqual(expectedSops,after.sopInstanceUids))throw new HttpError(502,"Patient Identity Reconciliation verification failed.",{code:"PATIENT_IDENTITY_RECONCILIATION_VERIFICATION_FAILED"});
}
...
export async function processClaimedPatientIdentityReconciliationJob(input:{job:PatientIdentityReconciliationJob;leaseOwner:string;leaseSeconds:number}):Promise<void>{
  ...
  try {
    ...
    if(!savedBefore){
      if(job.operation_type==="reverse" && (!original || !equal(before.otherPatientIdsSequence,original.result_other_patient_ids) || !idsContained(original.original_series_instance_uids,before.seriesInstanceUids) || !idsContained(original.original_sop_instance_uids,before.sopInstanceUids))) throw new HttpError(409,"Manual review required: study state changed after reconciliation.",{code:"PATIENT_IDENTITY_RECONCILIATION_MANUAL_REVIEW_REQUIRED"});
      ...
      await pool.query(`update patient_identity_reconciliation_jobs set orthanc_study_id_before=$2,old_patient_id=$3,new_patient_id=$4,original_other_patient_ids=$5,result_other_patient_ids=$6,original_patient_name=$7,original_patient_birth_date=$8,original_patient_sex=$9,original_series_instance_uids=$10,original_sop_instance_uids=$11,accession_number=coalesce(accession_number,$12),study_date=$13,updated_at=now() where id=$1 and lease_owner=$14 and status='processing'`,[job.id,before.orthancStudyId,before.patientId,target,JSON.stringify(before.otherPatientIdsSequence),JSON.stringify(sequence),before.patientName,before.patientBirthDate,before.patientSex,JSON.stringify(before.seriesInstanceUids),JSON.stringify(before.sopInstanceUids),before.accessionNumber,before.studyDate,leaseOwner]);
    }
    ...
    if(savedBefore)verifyRecoveredTarget(before,after,target,sequence,job.id,Boolean(existingAudit));else verify(before,after,target,sequence);
...
export const __patientIdentityReconciliationTestables={appendHistoricalPatientId,verify,verifyRecoveredTarget,auditUid,persistedSnapshot};
