...
export type IrReferralScheduleRequest = {
  ...
  requestedProcedure: string; assignedDoctorName: string | null; requestedModalityId: number; requestedModalityNameAr: string | null; requestedModalityNameEn: string | null; requestedExamTypeId: number; requestedExamTypeNameAr: string | null; requestedExamTypeNameEn: string | null; preferredDate: string | null;
...
const SCHEDULE_SELECT = `select sr.id, sr.ir_referral_case_id as "irReferralCaseId", ir.patient_id as "patientId", p.mrn as "patientMrn", p.english_full_name as "patientEnglishName", p.arabic_full_name as "patientArabicName", ir.requested_procedure as "requestedProcedure", coalesce(assigned_user.english_name, assigned_user.full_name, assigned_doctor.display_name) as "assignedDoctorName", sr.requested_modality_id as "requestedModalityId", requested_modality.name_ar as "requestedModalityNameAr", requested_modality.name_en as "requestedModalityNameEn", sr.requested_exam_type_id as "requestedExamTypeId", requested_exam.name_ar as "requestedExamTypeNameAr", requested_exam.name_en as "requestedExamTypeNameEn", sr.preferred_date as "preferredDate", sr.urgency, sr.reception_instruction as "receptionInstruction", sr.technologist_instruction as "technologistInstruction", sr.status, sr.appointment_id as "appointmentId", sr.requested_at as "requestedAt", sr.scheduled_at as "scheduledAt" from ir_referral_schedule_requests sr join ir_referral_cases ir on ir.id = sr.ir_referral_case_id join patients p on p.id = ir.patient_id left join doctor_portal.doctor_profiles assigned_doctor on assigned_doctor.id = ir.assigned_doctor_id left join users assigned_user on assigned_user.id = assigned_doctor.user_id left join modalities requested_modality on requested_modality.id = sr.requested_modality_id left join exam_types requested_exam on requested_exam.id = sr.requested_exam_type_id`;
...
async function requireIrScheduleReviewer(actor: IrReferralActor, referral: IrReferralRow) { const profile = await findActiveDoctorProfileByUserId(actor.userId); if (!profile) throw new HttpError(403, "An active Doctor Workspace profile is required."); if (Number(referral.assignedDoctorId) !== Number(profile.id) && !MANAGER_ROLES.has(actor.appRole)) throw new HttpError(403, "Only the assigned IR doctor can request scheduling."); return profile; }
