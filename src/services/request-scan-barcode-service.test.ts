...
test("accepts single-page Code 128 and Code 39 V2 accessions", () => { assert.deepEqual(interpretRequestScanBarcodes("CODE-128:V2-003628\n"), { ok: true, accessions: ["V2-003628"], accession: "V2-003628", qrTokens: [] }); assert.deepEqual(interpretRequestScanBarcodes("CODE-39:v2-003628\n"), { ok: true, accessions: ["V2-003628"], accession: "V2-003628", qrTokens: [] }); });
...
test("QR-only evidence skips 600 DPI after the complete 300-DPI original sweep", async () => {
  ...
  assert.deepEqual(pdf, { ok: true, accessions: [], qrTokens: [QR_TOKEN] });
  assert.deepEqual(renders, [300]);
});
test("uses the unchanged upright image as the original fast path", async () => { await withImage(async (image) => { const calls: string[] = []; assert.deepEqual(await extractRequestScanBarcode(image, dependencies(() => "CODE-128:V2-003628", { calls })), { ok: true, accessions: ["V2-003628"], accession: "V2-003628", qrTokens: [] }); assert.deepEqual(calls, [image]); }); });
test("uses preprocessing and each bounded rotation when needed", async () => { for (const angle of [0, 90, 180, 270]) await withImage(async (image) => { const result = await extractRequestScanBarcode(image, dependencies((filePath) => angle === 0 ? (filePath.includes("processed-1.png") && !filePath.includes("rotated") ? "CODE-128:V2-003628" : noSymbol()) : (filePath.includes(`rotated-${angle}.png`) ? "CODE-39:V2-003628" : noSymbol()))); assert.deepEqual(result, { ok: true, accessions: ["V2-003628"], accession: "V2-003628", qrTokens: [] }); }); });

test("does not render 600 DPI after a 300-DPI success or ambiguity", async () => {
  const successRenders: number[] = []; assert.deepEqual(await extractRequestScanBarcode("success.pdf", dependencies((filePath) => filePath.includes("pdf-300") && filePath.endsWith("page-1.png") ? "CODE-128:V2-003628" : noSymbol(), { renders: successRenders })), { ok: true, accessions: ["V2-003628"], accession: "V2-003628", qrTokens: [] }); assert.deepEqual(successRenders, [300]);
  const ambiguousRenders: number[] = []; const ambiguousCalls: string[] = []; assert.deepEqual(await extractRequestScanBarcode("ambiguous.pdf", dependencies((filePath) => filePath.endsWith("page-1.png") ? "CODE-39:V2-003628" : filePath.endsWith("page-2.png") ? "CODE-128:V2-003629" : noSymbol(), { renders: ambiguousRenders, calls: ambiguousCalls })), { ok: true, accessions: ["V2-003628", "V2-003629"], qrTokens: [] }); assert.deepEqual(ambiguousRenders, [300]); assert.equal(ambiguousCalls.filter((filePath) => /page-\d+\.png$/i.test(filePath)).length, 2);
});
...
test("uses enhanced PDF processing only after an empty original sweep and retries 600 DPI exactly once only then", async () => {
  ...
  assert.deepEqual(enhanced, { ok: true, accessions: ["V2-003628"], accession: "V2-003628", qrTokens: [] });
  assert.deepEqual(renders, [300]);
...
test("lists and extracts native PDF images once while retaining conflicts across pages", async () => {
  ...
  assert.deepEqual(result, { ok: true, accessions: ["V2-003628", "V2-003629"], qrTokens: [] });
  assert.equal(pdfimagesArgs.filter((args) => args[0] === "-list").length, 1);
  assert.equal(pdfimagesArgs.filter((args) => args[0] !== "-list").length, 1);
});
test("retries a PDF once at 600 DPI and records a 600-DPI success", async () => { const renders: number[] = []; const calls: string[] = []; const diagnostics: Record<string, string | number | boolean>[] = []; const result = await extractRequestScanBarcode("fallback.pdf", dependencies((filePath) => filePath.includes("pdf-600") && filePath.includes("rotated-270") ? "CODE-128:V2-003628" : noSymbol(), { renders, calls, diagnostics })); assert.deepEqual(result, { ok: true, accessions: ["V2-003628"], accession: "V2-003628", qrTokens: [] }); assert.deepEqual(renders, [300, 600]); assert.ok(calls.some((filePath) => filePath.includes("pdf-600") && filePath.includes("rotated-270"))); assert.ok(diagnostics.some((entry) => entry.code === "BARCODE_SUCCESS_PDF_600_DPI")); assert.ok(diagnostics.some((entry) => entry.successfulDpi === 600 && entry.fallbackUsed === true)); });
test("invalid values at 300 DPI permit the 600-DPI fallback, while 600 ambiguity remains manual review", async () => { const renders: number[] = []; const result = await extractRequestScanBarcode("invalid-then-ambiguous.pdf", dependencies((filePath) => filePath.includes("pdf-300") ? "CODE-128:OTHER-1" : filePath.endsWith("page-1.png") ? "CODE-128:V2-003628" : filePath.endsWith("page-2.png") ? "CODE-128:V2-003629" : noSymbol(), { renders })); assert.deepEqual(result, { ok: true, accessions: ["V2-003628", "V2-003629"], qrTokens: [] }); assert.deepEqual(renders, [300, 600]); });
