...
import { CLINICAL_CATEGORIES, createIncident, getIncident, listIncidentEquipment, listIncidents, reviewIncident } from "./incident-service.js";
...
test("incident service enforces incident types, categories, ordering, lookup state, and review audit", async (t) => {
  ...
  const userId = Number((await pool.query<{ id: number }>("insert into users(username,full_name,english_name,password_hash,role,is_active) values($1,$2,$3,'x','supervisor',true) returning id", [`incident_${suffix}`, `Incident reporter ${suffix}`, `Reporter English ${suffix}`])).rows[0]!.id);
  const reviewerId = Number((await pool.query<{ id: number }>("insert into users(username,full_name,english_name,password_hash,role,is_active) values($1,$2,$3,'x','supervisor',true) returning id", [`incident_reviewer_${suffix}`, `Incident reviewer ${suffix}`, `Reviewer English ${suffix}`])).rows[0]!.id);
  ...
  try {
    ...
    const reviewed = await reviewIncident(equipmentIncident.id, { status: "resolved", reviewNotes: "corrected" }, reviewerId); assert.equal(reviewed.status, "resolved"); assert.equal(Number(reviewed.reviewed_by_user_id), reviewerId);
    await assert.rejects(() => reviewIncident(equipmentIncident.id, { status: "closed", reviewNotes: "" }, reviewerId));
    await pool.query("update users set full_name=$2, english_name=$3 where id=$1", [userId, "Renamed reporter", "Renamed reporter EN"]);
    await pool.query("update users set full_name=$2, english_name=$3 where id=$1", [reviewerId, "Renamed reviewer", "Renamed reviewer EN"]);
    const historical = await getIncident(equipmentIncident.id);
    assert.equal(historical.reporter_name_ar, `Incident reporter ${suffix}`);
    assert.equal(historical.reporter_name_en, `Reporter English ${suffix}`);
    assert.equal(historical.reviewer_name_ar, `Incident reviewer ${suffix}`);
    assert.equal(historical.reviewer_name_en, `Reviewer English ${suffix}`);
