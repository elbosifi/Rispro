...
/**
 * @typedef CalendarSummaryFilters
 * @property {string} dateFrom
 * @property {string} dateTo
 * @property {string} [modalityId]
 */
...
/**
 * @typedef CalendarDaySummary
 * @property {string} appointment_date
 * @property {number} total_count
 * @property {{ modality_id: number, modality_name_en: string, count: number }[]} modalities
 */

/**
 * Lightweight monthly calendar summary: grouped by date, with modality breakdown per day.
 * No patient details, no notes, no heavy joins. Pure aggregation.
 *
 * @param {CalendarSummaryFilters} filters
 * @returns {Promise<{ days: CalendarDaySummary[] }>}
 */
export async function listAppointmentsCalendarSummary(filters) {
  const dateFrom = normalizeAppointmentDate(filters.dateFrom, "dateFrom");
  const dateTo = normalizeAppointmentDate(filters.dateTo, "dateTo");
  const modalityId = filters.modalityId ? normalizePositiveInteger(filters.modalityId, "modalityId", { required: false }) : null;
  ...
  const { rows } = await pool.query(
    `
      with day_agg as (
        ...
        from appointments a
        join modalities m on m.id = a.modality_id
      ...
      select
        ...
        (
          select jsonb_agg(
            jsonb_build_object(
              'modality_id', mr->>'modality_id',
              'modality_name_en', mr->>'modality_name_en',
              'count', mr->>'count'
            )
            order by (mr->>'count')::int desc
          )
          from jsonb_array_elements(modality_rows) mr
        ) as modalities
      from day_agg
...
/**
 * @typedef CalendarDayFilters
 * @property {string} date
 * @property {string} [modalityId]
 */

/**
 * Fetch detailed appointments for a single day (used by calendar sidebar).
 * Much lighter than listAppointmentsForPrint — no general instructions, no exam instructions.
 *
 * @param {CalendarDayFilters} filters
 * @returns {Promise<any[]>}
 */
export async function listAppointmentsByDate(filters) {
  const date = normalizeAppointmentDate(filters.date, "date");
  const modalityId = filters.modalityId ? normalizePositiveInteger(filters.modalityId, "modalityId", { required: false }) : null;
  ...
  return /** @type {any[]} */ (rows).map((row) => ({
    ...
    isOverbooked: row.is_overbooked,
    modalitySlotNumber: row.modality_slot_number,
