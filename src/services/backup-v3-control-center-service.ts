...
export async function getBackupControlCenterSummary() {
  ...
  const { rows } = await pool.query(
    `select
      ...
      (select row_to_json(verification) from (select status,created_at,started_at,completed_at,failure_message,artifact_id,destination_id from backup_restore_verification_jobs order by created_at desc limit 1) verification) as latest_restore_verification_attempt,
