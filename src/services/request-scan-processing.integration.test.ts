...
test("Request Scan document fingerprint migrations are repeatable and indexed", async (t) => { if (!(await ready(t))) return; for (const migration of ["152_modality_document_content_fingerprint.sql", "153_request_scan_reception_content_fingerprint_indexes.sql"]) { const sql = await fs.readFile(new URL(`../db/migrations/${migration}`, import.meta.url), "utf8"); await pool.query(sql); await pool.query(sql); } const column = await pool.query("select 1 from information_schema.columns where table_name='documents' and column_name='content_sha256'"); assert.equal(column.rowCount, 1); const indexes = await pool.query("select indexname from pg_indexes where indexname=any($1::text[]) order by indexname", [["documents_modality_clinical_fingerprint_idx","documents_modality_clinical_legacy_fingerprint_idx","documents_reception_request_fingerprint_idx","documents_reception_request_legacy_fingerprint_idx"]]); assert.equal(indexes.rowCount, 4); });
