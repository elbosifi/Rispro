...
test("v3 restore skeleton route does not contain destructive restore operations", async () => {
  ...
  assert.match(safetyService, /pg_dump", \["-Fc", "--file", dumpPath\]/);
  assert.doesNotMatch(safetyService, /BACKUP_V3_EPHEMERAL_TABLE_DATA_PG_DUMP_ARGS/);
  assert.match(combined, /pgDumpConnectionEnv\(env\.databaseUrl\)/);
  assert.match(combined, /streamBackupV3Archive/);
  assert.match(safetyService, /includeEphemeralTableData: true/);
