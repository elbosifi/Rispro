...
test("planned doctor activates on release, falls back when ineligible, and unassignment clears plans", async () => {
  ...
  const afterUnassign = await pool.query<{ count: string }>("select count(*)::text count from doctor_portal.comparison_case_assignments where comparison_request_id=$1 and status='active'", [request.id]); assert.equal(Number(afterUnassign.rows[0]!.count), 0);
  ...
  assert.equal(poolRelease.status, "ready_for_reporting"); assert.equal(poolRelease.assignedDoctorId, null); assert.equal(poolRelease.plannedReportingDoctorId, null); assert.equal(poolRelease.plannedReportingDoctorSetBy, null); assert.equal(poolRelease.plannedReportingDoctorSetAt, null);
  const fallbackAssignments = await pool.query<{ count: string }>("select count(*)::text count from doctor_portal.comparison_case_assignments where comparison_request_id=$1 and status='active'", [fallback.id]); assert.equal(Number(fallbackAssignments.rows[0]!.count), 0);
});
