...
async function eligibleAppointment(accession: string): Promise<EligibleAppointment> {
...
async function moveOutcome(settings: RequestScanSettings, job: RequestScanJob, folder: string, duplicate = false): Promise<string> { return moveRequestScanFile(settings, job.source_relative_path, destination(folder, duplicate), job.filename); }

export async function processRequestScanJob(jobId: number, suppliedSettings?: RequestScanSettings): Promise<RequestScanJob> {
  ...
  try {
    await downloadRequestScanFile(settings, job.source_relative_path, localPath);
    const barcode = await extractRequestScanBarcode(localPath);
    if (!barcode.ok) throw new HttpError(422, errorMessage[barcode.reason]);
    const appointment = await eligibleAppointment(barcode.accession);
    if (await automatedDocumentExists(appointment.id)) {
      const moved = await moveOutcome(settings, job, settings.processedSubfolder, true);
    ...
    const document = await uploadDocument({ patientId: appointment.patient_id, appointmentId: appointment.id, appointmentRefType: "v2_booking", documentType: "appointment_request", originalFilename: job.filename, mimeType: job.mime_type, fileContentBuffer: buffer, source: "request_scan_automation" }, null);
    const moved = await moveOutcome(settings, job, settings.processedSubfolder);
    ...
    try { moved = await moveOutcome(settings, job, settings.failedSubfolder); } catch { /* keep the original path so recovery can retry it */ }
...
export async function runRequestScanCycle(): Promise<{ processed: number; failed: number; duplicates: number; skipped: number }> {
  const settings = await readRequestScanSettings(); if (!settings.enabled) return { processed: 0, failed: 0, duplicates: 0, skipped: 0 };
  const files = await listRequestScanFiles(settings); const result = { processed: 0, failed: 0, duplicates: 0, skipped: 0 };
  for (const file of files) {
    ...
    const completed = await processRequestScanJob(job.id, settings); if (completed.status === "processed") result.processed += 1; else if (completed.status === "duplicate") result.duplicates += 1; else result.failed += 1;
  }
  return result;
}
...
export async function manuallyAssignRequestScan(id: number, appointmentId: number, userId: number): Promise<RequestScanJob> { const settings = await readRequestScanSettings(); const job = await getRequestScanJob(id); if (job.status !== "failed") throw new HttpError(409, "Only failed request scans can be assigned manually."); const appointment = await eligibleAppointment(`V2-${String(appointmentId).padStart(6, "0")}`); const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "rispro-request-scan-manual-")); try { const localPath = path.join(tempDir, job.filename); await downloadRequestScanFile(settings, job.source_relative_path, localPath); const document = await uploadDocument({ patientId: appointment.patient_id, appointmentId: appointment.id, appointmentRefType: "v2_booking", documentType: "appointment_request", originalFilename: job.filename, mimeType: job.mime_type, fileContentBuffer: await fs.readFile(localPath), source: "request_scan_automation" }, userId); const moved = await moveOutcome(settings, job, settings.processedSubfolder); return updateJob(id, { status: "processed", appointment_id: appointment.id, document_id: document.id, source_relative_path: moved, error_message: null, completed_at: new Date().toISOString() }); } finally { await fs.rm(tempDir, { recursive: true, force: true }); } }
