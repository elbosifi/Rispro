...
export async function claimBackupV3PreviewForRestore(id: string) { const client = await pool.connect(); try { await client.query("begin"); const { rows } = await client.query<PreviewRow>("select * from backup_restore_preview_jobs where preview_job_id=$1::uuid for update", [id]); const job = rows[0]; if (!job) throw new HttpError(404, "Restore preview job was not found."); if (job.status !== "succeeded" || job.expires_at <= new Date() || !job.archive_path || !job.archive_sha256 || job.archive_size_bytes == null) throw new HttpError(409, "Restore preview must be successful, unexpired, and unused."); await client.query("update backup_restore_preview_jobs set status='consumed',consumed_at=now() where preview_job_id=$1::uuid", [id]); await client.query("commit"); return { archivePath: job.archive_path, archiveSha256: job.archive_sha256, archiveSizeBytes: Number(job.archive_size_bytes), stagingDir: path.join(path.dirname(job.archive_path), "staged") }; } catch (error) { await client.query("rollback").catch(() => undefined); throw error; } finally { client.release(); } }
...
export async function cleanupBackupV3RestoreJobs() {
  ...
  const previews = await pool.query<{ preview_job_id: string; archive_path: string | null }>("select preview_job_id,archive_path from backup_restore_preview_jobs where cleanup_at is null and status='expired'");
