...
test("migration 181 declares incident lifecycle constraints, document restriction, and the existing updated-at trigger function", () => {
  ...
  assert.match(sql, /for\s+each\s+row\s+execute\s+function\s+touch_protocol_management_updated_at\s*\(\s*\)/i);
  assert.doesNotMatch(sql, /set_updated_at\s*\(\s*\)/i);
});
