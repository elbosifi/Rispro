...
export function setRequestScanFileHeaders(res: Response, job: Pick<RequestScanJob, "mime_type" | "filename">): void { res.setHeader("Content-Type", job.mime_type); res.setHeader("Content-Disposition", `inline; filename="${job.filename.replace(/"/g, "")}"`); res.setHeader("Cache-Control", "private, no-store"); }
