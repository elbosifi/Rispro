...
requestScansRouter.get("/", asyncRoute(async (req: Request, res: Response) => { res.json({ jobs: (await listRequestScanJobs(req.query.status)).map(withSafeRequestScanFilename) }); }));
