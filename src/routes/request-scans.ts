...
requestScansRouter.get("/eligible-appointments", asyncRoute(async (req: Request, res: Response, next) => {
  ...
  const query = buildModalityEligibleAppointmentsQuery(req.query.q);
  const { rows } = await pool.query(query.text, [scope.modalityId, ...query.values]);
