...
import { assertClinicalDocumentExportAppointmentAccess, listClinicalDocumentExportsForAppointment, reconcileClinicalDocumentExportsManually, retryClinicalDocumentExport } from "../services/clinical-document-export-service.js";
import {
...
authoritativeOrthancRouter.post("/document-exports/reconcile", requireAnyRole(["super_admin"]), asyncRoute(async (req: Request, res: Response) => { res.status(202).json(await reconcileClinicalDocumentExportsManually(req.user!.sub)); }));
