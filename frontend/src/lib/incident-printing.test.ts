...
describe("printIncidentReport", () => {
  ...
  it("renders a formal A4 LTR NCCB report with repeated print identity and safe pagination rules", () => {
    ...
    expect(html).toContain('lang="en" dir="ltr"'); expect(html).toContain('@page { size: A4 portrait; margin: 32mm 15mm 20mm; }'); expect(html).toContain('@bottom-right { content: "Page " counter(page) " of " counter(pages);'); expect(html).toContain(".institutional-header { position: fixed;"); expect(html).toContain(".report-footer { position: fixed;"); expect(html).toContain("https://rispro.test/assets/nccb-logo.png"); expect(html).toContain("National Cancer Center Benghazi"); expect(html).toContain("Diagnostic &amp; Interventional Radiology Department"); expect(html).toContain("Incident Report"); expect(html).toContain("INC-1"); expect(html).toContain("Description of incident"); expect(html).toContain("Immediate action taken"); expect(html).toContain("Administrative Review / Review Notes"); expect(html).toContain("MRI Scanner A"); expect(html).toContain("Reviewer"); expect(html).toContain("&lt;script&gt;"); expect(html).toContain("Stopped scanner\nPlaced warning notice"); expect(html).toContain("&lt;review&gt;"); expect(html).toContain("&lt;x&gt;.pdf"); expect(html).toContain("white-space: pre-wrap"); expect(html).toContain("Official Stamp"); expect(html).toContain('class="incident-number isolate"'); expect(html).toContain("printReportWhenReady"); expect(html.match(/<header class="institutional-header">/g)).toHaveLength(1); expect(html.match(/<div class="title-block">/g)).toHaveLength(1); expect(html).toContain('</header>\n  <div class="title-block">');
  });
  it("renders an Arabic RTL report with bilingual header, localized sections, and isolated MRN", () => {
    ...
    const html = String(write.mock.calls[0][0]);
    expect(html).toContain('lang="ar" dir="rtl"'); expect(html).toContain('@bottom-left { content: "الصفحة " counter(page) " من " counter(pages);'); expect(html).toContain(".institutional-header { position: fixed;"); expect(html).toContain(".report-footer { position: fixed;"); expect(html).toContain("المركز الوطني للأورام بنغازي"); expect(html).toContain("مريض عربي"); expect(html).toContain("فحص غير صحيح"); expect(html).toContain("MRN-1"); expect(html).toContain('class="isolate"'); expect(html).toContain("لا توجد مرفقات"); expect(html).toContain("قسم الأشعة التشخيصية والتداخلية"); expect(html).toContain("National Cancer Center Benghazi"); expect(html.match(/<header class="institutional-header">/g)).toHaveLength(1); expect(html.match(/<div class="title-block">/g)).toHaveLength(1);
  });
