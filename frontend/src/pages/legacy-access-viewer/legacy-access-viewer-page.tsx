...
async function fetchAppointments(params: Record<string, string>): Promise<{ appointments: AppointmentRow[] }> {
...
function printCurrentStyle(row: AppointmentRow): void {
...
function printLegacyStyle(row: AppointmentRow): void {
  const win = window.open("", "_blank");
  if (!win) return;
  const now = new Date().toLocaleString();
  win.document.write(`
...
export default function LegacyAccessViewerPage() {
  ...
  // Mount: check if an MDB file is already active
  useEffect(() => {
    fetchMdbStatus().then((res) => {
      ...
      if (res.status.hasActiveFile) {
        ...
        fetchAppointments({}).then(r => setAppointments(r.appointments)).catch(() => {});
      }
    }).catch(() => {});
  }, []);
  ...
  const handleFileUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      pushToast({ type: "success", title: "تم تحميل الملف", message: `تم تحميل ${file.name} بنجاح.` });
    } catch (err: unknown) {
  ...
  const handleSearch = useCallback(async () => {
    ...
    try {
      ...
      const normPatientName = patientName.trim();
      const normModality = modality.trim();
      const normExam = exam.trim();
  ...
  // -----------------------------------------------------------------------
  // Inline styles for compact desktop layout
  // -----------------------------------------------------------------------
  ...
  return (
    <div dir="rtl" style={{ fontFamily: "'Segoe UI', Tahoma, Arial, sans-serif", color: "#222", background: "#e8e8e8", minHeight: "100vh", padding: 8 }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6, padding: "4px 10px", background: "#0f766e", color: "#fff", borderRadius: 4 }}>
        ...
        <span style={{ fontSize: 10, opacity: 0.85 }}>Access MDB Viewer</span>
      </div>
