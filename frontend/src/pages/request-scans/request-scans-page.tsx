...
function ClinicalDocumentExportStatus({ language, job, canRetry, onRetry }: { language: Language; job: Job; canRetry: boolean; onRetry: () => void }) {
...
function RequestScanRow({ language, job, userRole, showClinicalExport, canRetryClinicalExports, showSelection, selected, openMenu, onSelect, onToggleMenu, onCloseMenu, onPreview, onDetails, onAction, onRetryClinicalExport, onAppointment, retryPending, stopping }: { language: Language; job: Job; userRole?: Role; showClinicalExport: boolean; canRetryClinicalExports: boolean; showSelection: boolean; selected: boolean; openMenu: boolean; onSelect: (checked: boolean) => void; onToggleMenu: () => void; onCloseMenu: () => void; onPreview: () => void; onDetails: () => void; onAction: (kind: RequestScanActionKind) => void; onRetryClinicalExport: () => void; onAppointment: () => void; retryPending: boolean; stopping: boolean }) {
  ...
  return <TableRow data-status={job.status} className={`${rowTone} transition-colors hover:bg-slate-50`}>
    ...
    <TableCell className="min-w-48 px-3 py-2"><RequestScanWorkflowStatus language={language} job={job} />{showClinicalExport ? <ClinicalDocumentExportStatus language={language} job={job} canRetry={canRetryClinicalExports} onRetry={onRetryClinicalExport} /> : null}</TableCell>
...
export default function RequestScansPage({ modality }: { modality?: { id: number; code: string; name: string; onBack: () => void; orthancState?: "connected" | "disabled" | "unavailable" } }) {
  ...
  const stopJob = useMutation({ mutationFn: (id: number) => request(`/${id}/stop`, { method: "POST" }), onSuccess: refresh, onError: (error: Error, id: number) => { setStoppingJobIds((ids) => ids.filter((jobId) => jobId !== id)); setNotice(localizeError(language, error)); } });
  ...
  const selectedAppointmentAllowed = Boolean(selectedAppointment && (!modality || selectedAppointment.modality_id == null || selectedAppointment.modality_id === modality.id));
  ...
  useEffect(() => { setStoppingJobIds((ids) => ids.filter((id) => jobs.data?.jobs.some((job) => job.id === id && job.status === "processing") ?? false)); }, [jobs.data?.jobs]);
