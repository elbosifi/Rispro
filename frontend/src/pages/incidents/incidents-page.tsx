...
import { Alert, AlertDescription, AlertTitle, Badge, Button, Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, EmptyState, ErrorState, Input, LoadingState, Textarea } from "@/components/shared";
...
const incidentActorName = (incident: Incident, role: "reporter" | "reviewer", language: "en" | "ar") => getUserDisplayName({
...
export default function IncidentsPage() {
  ...
  const equipmentReady = equipment.isSuccess && equipment.data.equipment.length > 0;
  ...
  const create = useMutation({
    ...
    onSuccess: async ({ incident: created }) => {
      ...
      resetCreateForm(); setAttachmentWarning(failed ? label("attachmentWarning") : ""); setReviewStatus(created.status); setReviewNotes(created.review_notes || ""); setFormOpen(false); setDetailId(created.id);
    },
  });
  const review = useMutation({
    mutationFn: () => reviewIncident(detailId!, { status: reviewStatus, reviewNotes }),
    onSuccess: async ({ incident: reviewed }) => { setReviewStatus(reviewed.status); setReviewNotes(reviewed.review_notes || ""); await Promise.all([queryClient.invalidateQueries({ queryKey: ["incident", detailId] }), queryClient.invalidateQueries({ queryKey: ["incidents"] })]); setReviewError(""); },
  });
  function resetCreateForm() {
    setKind("equipment"); setTerm(""); setDebouncedTerm(""); setSelectedPatient(null); setEquipmentId(""); setCondition(""); setVendorContacted(false); setFiles([]); setFileWarning(""); setAttachmentWarning(""); setOccurredError(""); setFormKey((value) => value + 1); create.reset();
  }
  ...
  return <div className="mx-auto flex w-full max-w-[1200px] flex-col gap-4 px-3 py-4 sm:px-4 sm:py-6" dir={language === "ar" ? "rtl" : "ltr"}>
    ...
    <section className="overflow-hidden rounded-2xl border border-border bg-card" aria-labelledby="incident-register-heading">
      ...
      {register.isLoading ? <LoadingState message={label("registerLoading")} /> : register.isError ? <div className="pb-5 text-center"><ErrorState message={label("registerError")} /><Button type="button" variant="secondary" size="sm" onClick={() => void register.refetch()}>{label("retry")}</Button></div> : !register.data?.incidents.length ? <EmptyState message={label("registerEmpty")} /> : register.data.incidents.map((item) => {
        ...
        return <button type="button" key={item.id} onClick={() => { setDetailId(item.id); setReviewStatus(item.status); setReviewNotes(item.review_notes || ""); }} className="grid w-full gap-3 border-b border-border p-4 text-start transition-colors last:border-b-0 hover:bg-muted/30 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring sm:p-5">
          ...
          <span className="line-clamp-2 text-sm leading-6 text-muted-foreground">{item.description}</span><span className="text-xs text-muted-foreground">{label("reporterDisplay")}: {incidentActorName(item, "reporter", language) || "-"}</span>
        </button>;
      })}
    </section>
    ...
    <Dialog open={formOpen} onClose={closeCreate}><DialogContent maxWidth="760px"><DialogHeader closeLabel={label("close")}><DialogTitle>{label("new")}</DialogTitle><DialogDescription>{label("createDescription")}</DialogDescription></DialogHeader>
      <form key={formKey} className="space-y-4" onSubmit={(event) => {
        ...
        <Panel title={label("sectionIncidentType")}><fieldset className="grid gap-2 sm:grid-cols-2"><legend className="sr-only">{label("type")}</legend>{(["equipment", "clinical_workflow"] as IncidentType[]).map((value) => <label key={value} className={`cursor-pointer rounded-xl border p-3 text-sm ${kind === value ? "border-primary bg-primary/5 ring-1 ring-primary" : "border-border bg-card"}`}><input type="radio" name="incidentType" value={value} required checked={kind === value} onChange={(event) => changeKind(event.target.value as IncidentType)} className="me-2" /><span className="font-medium">{label(value === "equipment" ? "equipment" : "clinical")}</span></label>)}</fieldset></Panel>
        ...
        {kind === "equipment" ? <Panel title={label("equipmentDetails")}><div className="space-y-4">
          {equipment.isLoading ? <p className="text-sm text-muted-foreground">{label("equipmentLoading")}</p> : equipment.isError ? <div className="space-y-2"><p role="alert" className="text-sm text-red-700">{label("equipmentLoadError")}</p><Button type="button" variant="secondary" size="sm" onClick={() => void equipment.refetch()}>{label("retry")}</Button></div> : !equipment.data?.equipment.length ? <p role="status" className="text-sm text-muted-foreground">{label("equipmentEmpty")}</p> : <div><label htmlFor="incident-equipment" className="mb-1 block text-sm font-medium">{label("equipmentName")} <span aria-hidden="true">*</span></label><select id="incident-equipment" name="equipmentId" required dir="ltr" className="input-premium w-full text-left [unicode-bidi:isolate]" value={equipmentId} onChange={(event) => setEquipmentId(event.target.value)}><option value="">{label("selectEquipment")}</option>{equipment.data.equipment.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></div>}
        ...
        <DialogFooter><Button type="button" variant="secondary" disabled={create.isPending} onClick={closeCreate}>{label("cancel")}</Button><Button type="submit" disabled={create.isPending || (kind === "equipment" && !equipmentReady)}>{create.isPending ? label("submitting") : label("submit")}</Button></DialogFooter>
      </form>
