...
export default function QzTrayPrintingSection() {
  ...
  return (
    <div className="space-y-5">
        ...
        return (
          <section key={profile.documentType} className="rounded-xl border border-border p-4">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              ...
              <Field label="Paper width (mm)"><input className="input-premium h-10 w-full" type="number" min="1" step="0.1" value={profile.paperWidthMm} onChange={(event) => updateProfile(profile.documentType, { paperWidthMm: Number(event.target.value) })} /></Field>
              <Field label="Paper height (mm)"><input className="input-premium h-10 w-full" type="number" min="1" step="0.1" value={profile.paperHeightMm} onChange={(event) => updateProfile(profile.documentType, { paperHeightMm: Number(event.target.value) })} /></Field>
      ...
      <div className="flex flex-wrap justify-end gap-2"><Button type="button" variant="secondary" onClick={() => setSettings(createDefaultQzPrinterSettings())}>Restore organization defaults</Button><Button type="button" onClick={() => { const saved = saveQzPrinterSettings(settings); setSettings(saved); pushToast({ type: "success", title: "Printer settings saved", message: "This workstation will use the configured QZ printer mappings." }); }}>Save settings</Button></div>
    </div>
  );
}
