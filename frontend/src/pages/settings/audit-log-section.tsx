...
export default function AuditLogSection({ onReAuthRequired }: { onReAuthRequired: (key: string[]) => void }) {
  ...
  return (
    <div className="relative space-y-4">
      ...
      {selectedEntry ? <AuditDetails entry={selectedEntry} language={language} closeRef={drawerCloseRef} onClose={closeDetails} /> : null}
    </div>
  );
}
...
function AuditDetails({ entry, language, closeRef, onClose }: { entry: AuditEntry; language: "ar" | "en"; closeRef: RefObject<HTMLButtonElement | null>; onClose: () => void }) {
