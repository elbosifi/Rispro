...
export default function SystemDiagnosticsSection({ onReAuthRequired }: { onReAuthRequired: (key: string[]) => void }) {
  ...
  const backupRestore = (summary.data?.backupRestore || {}) as Record<string, unknown>;
  ...
  return <div className="space-y-4">
    ...
    {tab === "overview" ? summary.isLoading ? <p className="description-center">Loading system diagnostics…</p> : <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <Card title="Backup V3" rows={{ "Last success": backupRestore.last_success, "Last failure": backupRestore.last_failure, "Active jobs": backupRestore.active_jobs, "Failed jobs (24h)": backupRestore.failed_jobs_24h, "Worker heartbeat": (backupRestore.worker as Record<string, unknown> | undefined)?.heartbeat_at, "Worker warning": (backupRestore.worker as Record<string, unknown> | undefined)?.last_failure_message, "Restore verification": (backupRestore.latest_restore_verification as Record<string, unknown> | undefined)?.status, "Retention deletes (24h)": backupRestore.retention_deletes_24h, "Retention failures (24h)": backupRestore.retention_failures_24h }} />
