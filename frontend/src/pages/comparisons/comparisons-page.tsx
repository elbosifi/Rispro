  ...
  return (
    <section className="rounded-lg border border-border bg-muted/20 p-3" aria-label={t(language, "comparisons.finalConfirmation")}>
      ...
      <div className="grid gap-2 text-sm">
        ...
        <label className="flex items-start gap-2"><input type="checkbox" checked={documentsAvailabilityConfirmed} onChange={(event) => setDocumentsAvailabilityConfirmed(event.target.checked)} /><span>{t(language, "comparisons.confirmDocuments")}</span></label>
    ...
    <article className="overflow-hidden rounded-xl border border-border bg-card shadow-md">
      ...
      <section className="grid gap-3 rounded-lg border border-border/70 bg-muted/30 p-3 md:grid-cols-3" aria-label={t(language, "comparisons.materialReadiness")}>
        ...
        <div className="space-y-1">
          ...
          <p className={`text-xs font-semibold ${row.documentCount > 0 ? "text-emerald-700" : "text-amber-700"}`}>{row.documentCount > 0 ? t(language, "comparisons.attachedCount", { count: row.documentCount }) : t(language, "comparisons.noneAttached")}</p>
