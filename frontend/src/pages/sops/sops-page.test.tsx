...
vi.mock("./sop-editor", () => ({
  ...
  SopStructuredEditor: ({ value, editable }: { value: { sections: Array<{ title: string; required: boolean }> }; editable: boolean }) => <div data-testid="sop-structured-editor">{value.sections.map((section) => <section key={section.title}><h3>{section.title}</h3>{editable ? <><button type="button" aria-label="RTL">RTL</button><button type="button" aria-label="LTR">LTR</button><div contentEditable role="textbox" aria-label={`${section.title} content`} /></> : null}</section>)}</div>,
  SopReadOnlyDocument: ({ value }: { value: { sections: Array<{ title: string }> } }) => <div data-testid="sop-read-only-document">{value.sections.map((section) => <section key={section.title}><h3>{section.title}</h3></section>)}</div>,
}));
