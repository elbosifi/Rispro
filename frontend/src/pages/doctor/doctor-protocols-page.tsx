        ...
        <SettingsTable emptyText="No appointments need protocol assignment." headers={["Date/time", "Patient", "Age/sex", "Modality", "Exam", "Category", "Notes", "Appointment status", "Protocol status", "Assigned protocol", "Actions"]}>
          {appointments.map((appointment) => (
            <tr key={appointment.appointmentId} onClick={() => openAssignmentModal(appointment.appointmentId)} className="cursor-pointer hover:bg-slate-50">
              ...
              <Cell><div className="flex flex-wrap items-center gap-1"><ProtocolStatusBadge assigned={appointment.assignment !== null} />{appointment.activeComplementaryRecall ? <Badge variant={appointment.activeComplementaryRecall.status === "pending_scheduling" ? "warning" : "info"} size="sm">Complementary recall · {appointment.activeComplementaryRecall.status === "pending_scheduling" ? "Pending scheduling" : "Scheduled"}</Badge> : null}{appointment.modalitySafetyWorkflowType === "mri_primary_implant_screening" ? <MriPrimaryScreeningBadges result={appointment.mriPrimaryScreeningResult} /> : null}</div></Cell>
...
function ProtocolAssignmentModal({
  ...
  const [cancelRecallDialogOpen, setCancelRecallDialogOpen] = useState(false);
  ...
  const cancelRecallMutation = useMutation({ mutationFn: () => cancelComplementaryRecallRequest(appointment.activeComplementaryRecall!.id), onSuccess: async (result) => { setCancelRecallDialogOpen(false); await queryClient.invalidateQueries({ queryKey: ["doctor", "protocoling"] }); pushToast({ type: "success", title: result.linkedAppointmentStillExists ? "Recall cancelled; booked appointment remains" : "Recall cancelled" }); } });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-black/45 p-2 sm:p-4" onClick={() => { if (!saving) requestClose(); }} role="presentation" data-testid="protocol-assignment-modal-backdrop">
      ...
      >
        <header className="sticky top-0 z-20 shrink-0 border-b bg-background px-3 py-2.5 sm:px-4" style={{ borderColor: "var(--border)" }}>
          <div className="grid min-w-0 grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-[minmax(220px,1.05fr)_minmax(0,1.65fr)_minmax(300px,1.45fr)] lg:items-center">
            <div className="min-w-0">
              ...
              {appointment.activeComplementaryRecall ? <Badge variant={appointment.activeComplementaryRecall.status === "pending_scheduling" ? "warning" : "info"} size="sm">Complementary recall · {appointment.activeComplementaryRecall.status === "pending_scheduling" ? "Pending scheduling" : "Scheduled"}</Badge> : null}
        ...
        <Dialog open={cancelRecallDialogOpen} onClose={() => !cancelRecallMutation.isPending && setCancelRecallDialogOpen(false)}><DialogContent maxWidth="520px"><DialogHeader><DialogTitle>Cancel Recall Request</DialogTitle><DialogDescription>{appointment.activeComplementaryRecall?.status === "scheduled" ? "Cancelling the clinical recall does not cancel the already-booked appointment. Cancel the appointment separately if it is no longer required." : "Cancel this clinical complementary recall request?"}</DialogDescription></DialogHeader>{cancelRecallMutation.isError ? <p role="alert" className="text-sm text-red-700">{(cancelRecallMutation.error as Error).message}</p> : null}<DialogFooter><Button variant="secondary" onClick={() => setCancelRecallDialogOpen(false)} disabled={cancelRecallMutation.isPending}>Keep recall</Button><Button variant="destructive" onClick={() => cancelRecallMutation.mutate()} disabled={cancelRecallMutation.isPending}>{cancelRecallMutation.isPending ? "Cancelling..." : "Cancel recall"}</Button></DialogFooter></DialogContent></Dialog>
