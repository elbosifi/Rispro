...
import { formatDateLy, formatDateTimeLy, tripoliDateTimeLocalToIso } from "@/lib/date-format";
...
import type { ComplementaryRecallQaClassification, ComplementaryRecallReasonCode } from "@/lib/api/complementary-recalls";
...
function ProtocolAssignmentModal({
  ...
  const [recallReasonCode, setRecallReasonCode] = useState<ComplementaryRecallReasonCode | "">("");
  const [recallQaClassification, setRecallQaClassification] = useState<ComplementaryRecallQaClassification | "">("");
  const [recallUrgency, setRecallUrgency] = useState<import("@/lib/api/complementary-recalls").ComplementaryRecallUrgency>("routine");
  const [recallDueAt, setRecallDueAt] = useState("");
  const [recallReportingDisposition, setRecallReportingDisposition] = useState<import("@/lib/api/complementary-recalls").ComplementaryRecallReportingDisposition>("supplement_original_report");
  ...
  const recallMutation = useMutation({ mutationFn: () => { if (!recallReasonCode || !recallQaClassification) throw new Error("Recall reason and QA classification are required."); return createComplementaryRecallRequest(appointment.appointmentId, { receptionInstruction: receptionInstruction.trim() || null, technologistInstruction: technologistInstruction.trim(), reasonCode: recallReasonCode, qaClassification: recallQaClassification, urgency: recallUrgency, dueAt: tripoliDateTimeLocalToIso(recallDueAt), reportingDisposition: recallReportingDisposition }); }, onSuccess: async () => { setRecallDialogOpen(false); setReceptionInstruction(""); setTechnologistInstruction(""); setRecallReasonCode(""); setRecallQaClassification(""); setRecallUrgency("routine"); setRecallDueAt(""); setRecallReportingDisposition("supplement_original_report"); await queryClient.invalidateQueries({ queryKey: ["doctor", "protocoling"] }); pushToast({ type: "success", title: "Additional imaging requested" }); } });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-black/45 p-2 sm:p-4" onClick={() => { if (!saving) requestClose(); }} role="presentation" data-testid="protocol-assignment-modal-backdrop">
      ...
      >
        ...
        <Dialog open={recallDialogOpen} onClose={() => !recallMutation.isPending && setRecallDialogOpen(false)}><DialogContent maxWidth="560px"><DialogHeader><DialogTitle>Request additional imaging</DialogTitle><DialogDescription>Original exam: {appointment.examTypeName ?? "Unspecified"} · {appointment.modalityCode}</DialogDescription></DialogHeader><div className="grid gap-3"><div className="grid gap-3 sm:grid-cols-2"><label className="grid gap-1 text-sm font-medium">Reason <select required aria-label="Recall reason" value={recallReasonCode} onChange={(event) => setRecallReasonCode(event.target.value as typeof recallReasonCode)} className="input-premium font-normal"><option value="">Select reason</option><option value="missing_sequence_phase">Missing sequence or phase</option><option value="incomplete_anatomical_coverage">Incomplete anatomical coverage</option><option value="motion_nondiagnostic_quality">Motion or nondiagnostic quality</option><option value="incorrect_protocol">Incorrect protocol</option><option value="incorrect_contrast_phase_timing">Incorrect contrast phase/timing</option><option value="additional_diagnostic_characterization">Additional diagnostic characterization</option><option value="technical_equipment_problem">Technical equipment problem</option><option value="patient_related_limitation">Patient-related limitation</option><option value="other">Other</option></select></label><label className="grid gap-1 text-sm font-medium">QA classification <select required aria-label="QA classification" value={recallQaClassification} onChange={(event) => setRecallQaClassification(event.target.value as typeof recallQaClassification)} className="input-premium font-normal"><option value="">Select classification</option><option value="diagnostic_addition">Diagnostic addition</option><option value="technical_repeat">Technical repeat</option><option value="protocol_error">Protocol error</option><option value="acquisition_error">Acquisition error</option><option value="equipment_failure">Equipment failure</option><option value="patient_related_unavoidable">Patient-related unavoidable</option><option value="other">Other</option></select></label><label className="grid gap-1 text-sm font-medium">Urgency <select aria-label="Urgency" value={recallUrgency} onChange={(event) => setRecallUrgency(event.target.value as typeof recallUrgency)} className="input-premium font-normal"><option value="same_day">Same day</option><option value="within_24_hours">Within 24 hours</option><option value="within_72_hours">Within 72 hours</option><option value="routine">Routine</option></select></label><label className="grid gap-1 text-sm font-medium">Due date/time <Input type="datetime-local" value={recallDueAt} onChange={(event) => setRecallDueAt(event.target.value)} /></label><label className="grid gap-1 text-sm font-medium sm:col-span-2">Reporting disposition <select aria-label="Reporting disposition" value={recallReportingDisposition} onChange={(event) => setRecallReportingDisposition(event.target.value as typeof recallReportingDisposition)} className="input-premium font-normal"><option value="supplement_original_report">Supplement original report</option><option value="separate_report">Separate report</option><option value="no_separate_report">No separate report</option></select></label></div><label className="grid gap-1 text-sm font-medium">Reception instruction <Textarea value={receptionInstruction} onChange={(event) => setReceptionInstruction(event.target.value)} className="min-h-20 font-normal" /></label><label className="grid gap-1 text-sm font-medium">Technologist instruction <Textarea required value={technologistInstruction} onChange={(event) => setTechnologistInstruction(event.target.value)} className="min-h-24 font-normal" /></label>{recallMutation.isError ? <p role="alert" className="text-sm text-red-700">{(recallMutation.error as Error).message}</p> : null}</div><DialogFooter><Button variant="secondary" onClick={() => setRecallDialogOpen(false)} disabled={recallMutation.isPending}>Cancel</Button><Button onClick={() => recallMutation.mutate()} disabled={!technologistInstruction.trim() || !recallReasonCode || !recallQaClassification || recallMutation.isPending}>{recallMutation.isPending ? "Requesting..." : "Request additional imaging"}</Button></DialogFooter></DialogContent></Dialog>
        <Dialog open={withdrawRecallDialogOpen} onClose={() => !withdrawRecallMutation.isPending && setWithdrawRecallDialogOpen(false)}><DialogContent maxWidth="520px"><DialogHeader><DialogTitle>Withdraw request</DialogTitle><DialogDescription>Withdraw this pending additional imaging request?</DialogDescription></DialogHeader>{withdrawRecallMutation.isError ? <p role="alert" className="text-sm text-red-700">{(withdrawRecallMutation.error as Error).message}</p> : null}<DialogFooter><Button variant="secondary" onClick={() => setWithdrawRecallDialogOpen(false)} disabled={withdrawRecallMutation.isPending}>Keep request</Button><Button variant="destructive" onClick={() => withdrawRecallMutation.mutate()} disabled={withdrawRecallMutation.isPending}>{withdrawRecallMutation.isPending ? "Withdrawing..." : "Withdraw request"}</Button></DialogFooter></DialogContent></Dialog>
