...
function mriSequencePresetLabel(preset: Pick<MriSequencePreset, "name" | "defaultPlane" | "weighting" | "genericFamily" | "fatSuppression" | "acquisitionType" | "contrastRelation">): string {
  ...
  const details = [preset.acquisitionType, preset.contrastRelation].filter(Boolean).join(" Â· ");
  return [clinical || preset.name, details].filter(Boolean).join(" Â· ");
}
...
function mriSequenceRowLabel(row: ProtocolLibraryMriSequenceRow): string {
  ...
  const details = [row.presetAcquisitionType, row.presetContrastRelation].filter(Boolean).join(" Â· ");
  return [clinical || row.mriSequencePresetName || "-", details].filter(Boolean).join(" Â· ");
}
      ...
      <section className="rounded-lg border p-4" style={{ backgroundColor: "var(--card)", borderColor: "var(--border)" }}>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>{detail.protocol.modality} Â· {detail.protocol.anatomyRegionName ?? "No region"} Â· Version {detail.version.versionNumber} <VersionBadge status={detail.version.status} /></p>
          </div>
          <div className="flex flex-wrap gap-2">
...
function CtProtocolRowForm({ draft, presets, setDraft, onSave, onCancel }: { draft: ProtocolLibraryCtPhaseRowPayload; presets: CtPhasePreset[]; setDraft: (draft: ProtocolLibraryCtPhaseRowPayload | null) => void; onSave: () => void; onCancel: () => void }) {
  ...
  return (
    <tr><td colSpan={9} className="border-b p-3" style={{ borderColor: "var(--border)" }}><div className="grid gap-3 md:grid-cols-4">
      ...
      {selectedPreset && <p className="text-xs md:col-span-4" style={{ color: "var(--text-muted)" }}>Preset reference: {selectedPreset.contrastStatus} Â· {selectedPreset.timingType} Â· {selectedPreset.defaultCoverage ?? "No default coverage"}</p>}
    </div></td></tr>
  );
}
...
function MriProtocolRowForm({ draft, scanners, presets, setDraft, onSave, onCancel }: { draft: ProtocolLibraryMriSequenceRowPayload; scanners: ImagingScanner[]; presets: MriSequencePreset[]; setDraft: (draft: ProtocolLibraryMriSequenceRowPayload | null) => void; onSave: () => void; onCancel: () => void }) {
  ...
  return (
    <tr><td colSpan={9} className="border-b p-3" style={{ borderColor: "var(--border)" }}><div className="grid gap-3 md:grid-cols-4">
      ...
      {selectedPreset && <p className="text-xs md:col-span-4" style={{ color: "var(--text-muted)" }}>Preset reference: {mriSequencePresetLabel(selectedPreset)} Â· {selectedPreset.defaultCoverage ?? "No default coverage"} Â· {selectedPreset.defaultBValues ?? "No b-values"}</p>}
        ...
        <SettingsTable emptyText="No appointments need protocol assignment." headers={["Date/time", "Patient", "Age/sex", "Modality", "Exam", "Category", "Notes", "Protocol status", "Assigned protocol", "Actions"]}>
          {appointments.map((appointment) => (
            <tr key={appointment.appointmentId} onClick={() => openAssignmentModal(appointment.appointmentId)} className="cursor-pointer hover:bg-slate-50">
              ...
              <Cell>{appointment.assignment ? (appointment.assignment.freeTextProtocol ? "Free-text protocol" : `${appointment.assignment.protocolName ?? "Saved protocol"} v${appointment.assignment.versionNumber ?? "-"}`) + (appointment.assignment.scannerName ? ` Â· ${appointment.assignment.scannerName}` : "") : "-"}</Cell>
...
function ProtocolAssignmentModal({
  ...
  const protocolOptionLabel = (protocol: ProtocolLibraryProtocol) => `${protocol.name} Â· ${protocol.modality} Â· v${protocol.activeVersionNumber}`;
          ...
          <div className="grid min-w-0 grid-cols-1 gap-2 md:grid-cols-2 lg:grid-cols-[minmax(220px,1.05fr)_minmax(0,1.65fr)_minmax(300px,1.45fr)] lg:items-center">
            <div className="min-w-0">
              ...
              <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px]">
                <span><span className="font-semibold text-muted-foreground">Age / sex</span> <span className="font-semibold text-foreground">{appointment.ageYears ?? "â€”"} / {appointment.sex ?? "â€”"}</span></span>
                <span dir="ltr"><span className="font-semibold text-muted-foreground">Primary ID</span> <span className="font-semibold text-foreground">{appointment.patientDicomId || "â€”"}</span></span>
                <span dir="ltr"><span className="font-semibold text-muted-foreground">MRN</span> <span className="font-semibold text-foreground">{appointment.patientMrn || "â€”"}</span></span>
              </div>
              </div>
              <div className="relative flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 text-[11px]">
                <span dir="ltr"><span className="font-semibold text-muted-foreground">Appointment</span> <span className="font-semibold text-foreground">{formatDateLy(appointment.appointmentDate)} Â· {appointment.appointmentTime?.slice(0, 5) || "â€”"}</span></span>
              ...
              <div className={`min-h-0 min-w-0 overflow-hidden ${documentExpanded ? "lg:col-span-2" : ""}`}>
               ...
               {!documentExpanded ? (historyOpen ? <aside className="min-h-0 overflow-y-auto rounded-xl border p-3" style={{ borderColor: "var(--border)", backgroundColor: "var(--card)" }}>
                ...
                <div className="mt-3 rounded-lg border" style={{ borderColor: "var(--border)" }}>
                  <button type="button" className="flex w-full items-center justify-between px-3 py-2 text-start text-sm font-semibold" aria-expanded={additionalInstructionsOpen} onClick={() => setAdditionalInstructionsOpen((current) => !current)}>Additional instructions<span aria-hidden="true">{additionalInstructionsOpen ? "âˆ’" : "+"}</span></button>
...
function ProtocolAssignmentSummary({ detail }: { detail: DoctorProtocolingAppointmentDetail }) {
  ...
  return (
    <div className="mt-6 border-t pt-4" style={{ borderColor: "var(--border)" }}>
      ...
      <p className="mt-1 text-sm" style={{ color: "var(--text-muted)" }}>{assignment.freeTextProtocol ? "Free-text protocol" : `${assignment.protocolName ?? "Saved protocol"} v${assignment.versionNumber ?? "-"}`}{assignment.scannerName ? ` Â· ${assignment.scannerName}` : ""}</p>
