               ...
               {!documentExpanded ? (historyOpen ? <aside className="min-h-0 overflow-y-auto rounded-xl border p-3" style={{ borderColor: "var(--border)", backgroundColor: "var(--card)" }}>
                ...
                <div className="mt-3 flex flex-wrap gap-1.5">
                 ...
                 <section className="mt-4 border-t border-border pt-3" aria-label="Possible historical PACS matches">
                   <div className="flex flex-wrap items-center gap-2"><h5 className="text-sm font-semibold">Possible historical PACS matches</h5>{historicalCandidatesQuery.isFetching && historicalCandidatesQuery.data ? <span className="flex items-center gap-1 text-[11px] text-muted-foreground" role="status"><span className="h-3 w-3 animate-spin rounded-full border-2 border-current border-r-transparent" aria-hidden="true" />Refreshing historical PACS matches…</span> : null}</div>
                   ...
                   {historicalCandidatesQuery.isLoading && !historicalCandidatesQuery.data ? <div className="mt-3 rounded-lg border border-sky-300 bg-sky-50 p-3 text-sm text-sky-950" role="status" aria-live="polite"><div className="flex items-center gap-2 font-semibold"><span className="h-5 w-5 animate-spin rounded-full border-2 border-current border-r-transparent" aria-hidden="true" />Searching historical PACS matches…</div><p className="mt-2 text-xs">RISpro is comparing this patient with the historical PACS index.</p><p className="mt-1 text-xs font-medium">Patient history above is already available independently. Wait here if you want to see possible historical PACS matches.</p><p className="mt-1 text-xs">This search may take a few seconds.</p></div> : null}
                   {historicalCandidatesQuery.isError ? <div className="mt-3 rounded-lg border border-red-200 bg-red-50 p-3 text-xs text-red-800"><p className="font-semibold">Historical PACS search failed.</p><p className="mt-1">RISpro and PACS patient history above is still available.</p><Button type="button" variant="outline" size="sm" className="mt-2" onClick={() => historicalCandidatesQuery.refetch()}>Retry historical search</Button></div> : null}
                   ...
                   {historicalCandidatesQuery.data && historicalCandidatesQuery.data.historicalCandidates.length === 0 && !historicalCandidatesQuery.isError ? <p className="mt-3 text-xs text-muted-foreground">Historical PACS search complete. No possible matches were found.</p> : null}
