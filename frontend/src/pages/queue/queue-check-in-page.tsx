...
export default function QueueCheckInPage() {
  ...
  const modalityRows = useMemo<ModalityRow[]>(() => {
    ...
    for (const row of statistics?.modalityBreakdown ?? []) {
      upsert({
        key: normalizeKey(row.modalityNameEn, row.modalityNameAr),
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-[1600px] flex-col gap-4 p-4 sm:p-6 lg:p-8">
        <header className="rounded-[1.75rem] border border-border/70 bg-white/85 px-4 py-3 shadow-[0_18px_48px_rgba(15,23,42,0.08)] backdrop-blur-md sm:px-5">
          <div className={`flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between ${isArabic ? "xl:flex-row-reverse" : ""}`}>
            <div className={`flex items-center gap-3 ${isArabic ? "flex-row-reverse" : ""}`}>
              <div className="flex h-12 w-12 items-center justify-center overflow-hidden rounded-2xl border border-slate-200 bg-slate-50 shadow-sm">
                {!logoFailed ? (
                  <img
                    src="/nccb-logo.png"
                    alt={hospitalName}
              ...
              <div className="space-y-1">
                <p className="text-[10px] uppercase tracking-[0.26em] text-muted-foreground">{t("queue.checkInLabel")}</p>
                <h1 className="font-display text-xl font-semibold tracking-tight text-foreground sm:text-[1.6rem]">
                  {hospitalName}
                </h1>
                <p className="text-xs text-muted-foreground sm:text-sm">{pageTitle}</p>
              </div>
            </div>
            ...
            <div className={`flex flex-wrap items-center gap-2 ${isArabic ? "flex-row-reverse" : ""}`}>
              <div className="rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2">
                ...
                <p className="mt-0.5 text-base font-semibold text-foreground">{currentClock}</p>
              </div>

              <Button variant="ghost" size="sm" onClick={toggleLanguage} className="rounded-2xl px-3">
                {isArabic ? "EN" : "عربي"}
              </Button>
        ...
        <main className="grid flex-1 gap-4 xl:grid-cols-12">
          <section className="xl:col-span-12">
            <Card className="rounded-[1.5rem] border border-slate-200/80 bg-white/90 p-4 shadow-[0_14px_34px_rgba(15,23,42,0.06)] sm:p-5">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">{t("queue.checkInMetrics")}</p>
                  <h3 className="mt-0.5 text-lg font-semibold text-foreground">{t("queue.checkInTodayOverview")}</h3>
                </div>
              ...
              <div className="mt-3 flex items-center justify-between gap-3 rounded-2xl bg-slate-50 px-4 py-2 text-xs text-muted-foreground">
          ...
          <section className="flex min-h-0 flex-col gap-4 xl:col-span-4">
            <section className="relative overflow-hidden rounded-[1.75rem] border border-white/20 bg-[linear-gradient(135deg,rgba(0,82,255,0.97),rgba(10,132,255,0.92)_45%,rgba(15,118,110,0.92))] p-4 text-white shadow-[0_20px_50px_rgba(0,82,255,0.22)] sm:p-5">
              ...
              <div className="relative z-10 space-y-3">
                <div className={`flex items-center gap-3 text-white/[0.92] ${isArabic ? "flex-row-reverse" : ""}`}>
                  <span className="flex h-9 w-9 items-center justify-center rounded-2xl bg-white/[0.18] shadow-sm">
                    <ScanLine size={24} />
                  </span>
                  <div>
                    <p className="text-xs uppercase tracking-[0.28em] text-white/[0.72]">{t("queue.checkInReadyToScan")}</p>
                    <h2 className="mt-1 text-lg font-semibold tracking-tight sm:text-[1.4rem]">
                ...
                <div
                  className={`rounded-[1.35rem] border border-white/[0.16] px-4 py-3 ${
                    state.mode === "error"
                      ? "bg-rose-500/10"
                      : state.mode === "success"
                        ? "bg-emerald-500/10"
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {state.mode === "error" ? (
                      <TriangleAlert size={22} className="text-rose-100" />
                    ) : state.mode === "success" ? (
                      <BadgeCheck size={22} className="text-emerald-100" />
                    ) : (
                      <Waves size={22} className="text-white/[0.8]" />
                    )}
                  </div>

                  <p className="mt-2 text-sm leading-6 text-white/[0.88]">
                    {state.mode === "error"
                      ? state.message
                      : state.mode === "success"
                        ? (state.entry
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <Button
                    ...
                    className="rounded-full border border-white/[0.18] bg-white/[0.10] px-5 text-white hover:bg-white/[0.18]"
                  >
            ...
            <Card className="rounded-[1.5rem] border border-slate-200/80 bg-white/92 p-4 shadow-[0_14px_34px_rgba(15,23,42,0.06)]">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">{t("queue.checkInInstructionsTitle")}</p>
                  <h3 className="mt-0.5 text-lg font-semibold text-foreground">{t("queue.checkInInstructionsHeading")}</h3>
                </div>
          ...
          <section className="flex min-h-0 flex-col gap-4 xl:col-span-8">
            <Card className="rounded-[1.5rem] border border-slate-200/80 bg-white/90 p-4 shadow-[0_14px_34px_rgba(15,23,42,0.06)]">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">{t("queue.modalityStatus")}</p>
                  <h3 className="mt-0.5 text-lg font-semibold text-foreground">{t("queue.modalityStatusHeading")}</h3>
                </div>
                <Building2 className="h-4 w-4 text-[var(--accent)]" />
              </div>

              <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-2 2xl:grid-cols-3">
                {modalityRows.length > 0 ? (
                  modalityRows.map((row) => (
                    <div
                      key={row.key}
                      className="rounded-[1.25rem] border border-slate-200 bg-slate-50/80 p-4 shadow-sm"
                    >
                      <div className={`flex items-start justify-between gap-3 ${isArabic ? "flex-row-reverse" : ""}`}>
                        ...
                        <div className="min-w-14 rounded-2xl bg-[linear-gradient(135deg,rgba(0,82,255,0.12),rgba(14,165,233,0.12))] px-3 py-2 text-center">
                          <p className="text-[10px] uppercase tracking-[0.22em] text-muted-foreground">{t("queue.inQueue")}</p>
                          <p className="text-xl font-semibold text-[var(--accent)]">{row.inQueueCount}</p>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 grid grid-cols-3 gap-2 text-center text-sm">
                        <div className="rounded-xl bg-white px-3 py-2">
                          ...
                          <p className="mt-1 text-base font-semibold text-foreground">{row.inQueueCount}</p>
                        </div>
                        <div className="rounded-xl bg-white px-3 py-2">
                          <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">{t("queue.completed")}</p>
                          <p className="mt-1 text-base font-semibold text-foreground">{row.completedCount}</p>
                        </div>
                        <div className="rounded-xl bg-white px-3 py-2">
                          <p className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">{t("queue.total")}</p>
                          <p className="mt-1 text-base font-semibold text-foreground">{row.totalCount}</p>
                        </div>
                      </div>
                    </div>
                  ))
...
function StatTile({ label, value, accent }: { label: string; value: number; accent: string }) {
  return (
    <div className="rounded-2xl border border-slate-200 bg-white px-3 py-3 shadow-sm">
      <div className={`mb-2 h-2 w-10 rounded-full ${accent}`} />
