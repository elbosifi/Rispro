      ...
      >
        ...
        */}
        <div className="relative min-h-0 flex-1 overflow-hidden p-2 sm:p-3" aria-busy={appointmentQuery.isLoading}>
          ...
          {appointment && activeTab === "documents" ? <div className="h-full min-h-0"><RequestDocumentsPanel appointmentId={appointment.id} patientId={appointment.patientId} appointmentRefType="v2_booking" title={t("registrations.requestDocuments")} previewMode="inline" enableLocalScan layout="workspace" workspaceRailSize="wide" compactMobileWorkspace supplementaryPanelPlacement="before-documents" pdfUtilityToolbarPlacement="top" pdfInitialSizingMode="fit-width" hideSatisfiedProtocolEligibilityStatus supplementaryPanel={<>{protocolPanel}{reportPanel}</>} expanded={documentReviewExpanded} onExpandedChange={setDocumentReviewExpanded} onDocumentsChanged={() => void queryClient.invalidateQueries({ queryKey: ["registrations"] })} /></div> : null}
