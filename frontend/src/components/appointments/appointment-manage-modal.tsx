...
export type AppointmentManageModalProps = {
  ...
  /** Reuses the Recall Requests row when that workflow opened this modal. */
...
function reportingStatusBadge(language: "ar" | "en", status: AppointmentWithDetails["reportStatus"]) {
  ...
  const variants = { final: "success", draft: "warning", no_report: "neutral", study_not_found: "warning", unavailable: "neutral" } as const;
...
function AppointmentHeaderBadgeCluster({ appointment, language, compact = false }: { appointment: AppointmentWithDetails; language: "ar" | "en"; compact?: boolean }) {
  ...
  return <div data-testid="appointment-header-badge-cluster" className={`flex min-w-0 flex-wrap items-center ${compact ? "gap-1.5" : "gap-2"}`}>
    ...
    {appointment.reportingHold ? <Badge size={compact ? "sm" : "default"} variant="warning" className="whitespace-nowrap">{chooseLocalized(language, "تعليق التقارير", "Reporting hold")}</Badge> : null}
...
export function AppointmentManageModal({
  ...
  const [rescheduleRequestOverrideTypes, setRescheduleRequestOverrideTypes] = useState<SchedulingOverrideType[]>([]);
  ...
  const [isMobileViewport, setIsMobileViewport] = useState(() => typeof window !== "undefined" && typeof window.matchMedia === "function" && window.matchMedia("(max-width: 767px)").matches);
  ...
  useEffect(() => {
    if (!open) {
      protocolPanelContextKeyRef.current = null;
      return;
    }
    if (!appointment) return;
    const contextKey = `${appointment.id}:${isMobileViewport ? "mobile" : "desktop"}`;
    if (protocolPanelContextKeyRef.current === contextKey) return;
    protocolPanelContextKeyRef.current = contextKey;
    setProtocolPanelOpen(isMobileViewport ? Boolean(appointment.protocolAssignmentSummary || appointment.notes?.trim()) : true);
  }, [appointment, isMobileViewport, open]);
  ...
  const supplementaryRecallContext = appointment && appointment.complementaryImagingContext?.relationship ? (() => {
  ...
  return (
    ...
    <Dialog open={open} onClose={closeModal}>
      <DialogContent
        ...
        dir={isRtl ? "rtl" : "ltr"}
        className={`flex min-h-0 flex-col overflow-hidden !p-0 ${documentReviewExpanded ? "!m-2 !h-[calc(100dvh-16px)] !max-h-[calc(100dvh-16px)] !rounded-lg" : "!m-2 h-[calc(100dvh-16px)] !max-h-[calc(100dvh-16px)] !rounded-xl md:!mx-4 md:h-[94dvh] md:!max-h-[94dvh] md:!rounded-2xl"}`}
      >
        <DialogHeader closeLabel={t("toast.close")} className="sticky top-0 z-30 !mb-0 !items-start shrink-0 border-b border-border bg-background px-2.5 py-2 md:px-5 md:py-3">
          {appointment ? <div data-testid="compact-document-appointment-header" className="grid min-w-0 gap-2 md:gap-3 lg:grid-cols-[minmax(220px,1.1fr)_minmax(240px,1fr)_minmax(280px,1.4fr)_auto] lg:items-center">
            <div className="min-w-0"><DialogTitle className="truncate text-base font-semibold sm:text-lg">{appointment.arabicFullName || dialogTitle}</DialogTitle>{appointment.englishFullName && appointment.englishFullName !== appointment.arabicFullName ? <p className="mt-0.5 truncate text-xs text-muted-foreground" dir="ltr">{appointment.englishFullName}</p> : null}<p dir="ltr" className="mt-1 font-mono-data text-xs text-muted-foreground [unicode-bidi:isolate]">MRN {appointment.mrn || appointment.patientPrimaryIdentifierValue || "—"}</p></div>
            <div className="min-w-0 lg:border-s lg:border-border lg:ps-3"><p className="truncate text-sm font-semibold text-foreground sm:text-[15px]">{chooseLocalized(language, appointment.examNameAr, appointment.examNameEn) || "—"}</p><p className="mt-1 truncate text-xs text-muted-foreground">{chooseLocalized(language, appointment.modalityNameAr, appointment.modalityNameEn)} · {appointment.modalityCode || "—"}</p><p dir="ltr" className="mt-1 font-mono-data text-xs text-muted-foreground [unicode-bidi:isolate]">{appointment.accessionNumber}</p></div>
            <div className="min-w-0 lg:border-s lg:border-border lg:ps-3"><AppointmentHeaderBadgeCluster appointment={appointment} language={language} compact={isMobileViewport} /><p className="mt-1.5 md:mt-2 text-xs text-muted-foreground"><span dir="ltr" className="[unicode-bidi:isolate]">{formatDateLy(appointment.appointmentDate)} · {appointment.bookingTime || chooseLocalized(language, "الوقت غير محدد", "Time not assigned")}</span></p></div>
        ...
        */}
        <div className={`relative min-h-0 flex-1 ${mobileDocumentsWorkspace ? "overflow-x-hidden overflow-y-auto" : "overflow-hidden"} p-2 sm:p-3`} aria-busy={appointmentQuery.isLoading}>
          ...
          {appointment && activeTab === "details" ? <div data-testid="appointment-details-scroll-surface" className="absolute inset-2 z-30 overflow-y-auto rounded-xl border border-border bg-background p-3 shadow-xl sm:inset-3 sm:p-5"><AppointmentInformationView key={appointment.id} appointment={appointment} lookups={lookups} reportStatus={reportStatus} recallContext={recallContext} onBack={() => selectTab("documents")} onOpenPatientProfile={() => setSelectedPatientId(appointment.patientId)} onOpenReschedule={selectedCanReschedule ? () => selectTab("reschedule") : undefined} onOpenStatus={() => selectTab("status")} onOpenReport={openReport} onOpenAppointment={onOpenAppointment} onAppointmentUpdated={updateDisplayedAppointment} /></div> : null}
          ...
          {appointment && activeTab === "reschedule" ? (
            selectedCanReschedule ? <div className="absolute inset-2 z-20 overflow-y-auto rounded-xl border border-border bg-background p-3 shadow-xl sm:inset-3 sm:p-5"><div className="mb-3 flex items-center justify-between gap-2"><div><h4 className="text-sm font-semibold">{t("registrations.reschedule")}</h4><p className="mt-1 text-xs text-muted-foreground">{t("registrations.rescheduleHint")}</p></div><button type="button" className="rounded-lg p-2 text-muted-foreground hover:bg-muted" onClick={() => selectTab("documents")} aria-label={t("toast.close")}><X size={17} aria-hidden="true" /></button></div>
              {rescheduleAvailability.isError ? <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-700">{(rescheduleAvailability.error as Error | undefined)?.message || t("registrations.rescheduleAvailabilityFailed")}</div> : <form className="space-y-3" onSubmit={(event) => { event.preventDefault(); submitReschedule(); }}>
                ...
                <div className={`flex flex-wrap gap-2 ${isRtl ? "justify-start" : "justify-end"}`}>{user?.role === "receptionist" && allowReceptionOverrideRequestsFromAvailability && rescheduleSelectedRow && inferSupportedOverrideTypesFromExamRuleMetadata({ reasonCodes: rescheduleSelectedRow.reasonCodes, requiresSupervisorOverride: rescheduleSelectedRow.requiresSupervisorOverride, effectModes: rescheduleSelectedRow.matchedExamRuleSummary ? [rescheduleSelectedRow.matchedExamRuleSummary.effectMode] : [], capacityResolutionMode: rescheduleCapacityResolutionMode }).length > 0 ? <Button type="button" size="sm" variant="secondary" disabled={!canSubmitReschedule || createRescheduleOverrideRequest.isPending} onClick={() => { const payload = buildReschedulePayload(); if (!payload) return; setPendingReschedulePayload(payload); setRescheduleRequestOverrideTypes(inferSupportedOverrideTypesFromExamRuleMetadataRaw({ reasonCodes: rescheduleSelectedRow.reasonCodes, requiresSupervisorOverride: rescheduleSelectedRow.requiresSupervisorOverride, effectModes: rescheduleSelectedRow.matchedExamRuleSummary ? [rescheduleSelectedRow.matchedExamRuleSummary.effectMode] : [], capacityResolutionMode: effectiveRescheduleCapacityResolutionMode })); setRescheduleRequestError(null); setRescheduleRequestOpen(true); }}>{t("overrideRequests.requestApproval")}</Button> : null}<Button type="submit" size="sm" disabled={!canSubmitReschedule}>{rescheduleMutation.isPending ? t("appointments.v2.rescheduling") : t("registrations.reschedule")}</Button></div>
              </form>}
         ...
         {appointment && activeTab === "cancel" ? ["scheduled", "arrived", "waiting"].includes(appointment.status) ? <div className="rounded-2xl border border-border bg-muted/20 p-3"><div className="mb-2 text-xs text-muted-foreground">{t("registrations.cancelAppointment")}</div><div className={isRtl ? "flex justify-start" : "flex justify-end"}><Button size="sm" variant="ghost" style={{ color: "#ef4444" }} onClick={() => { if (window.confirm(t("common.confirmCancelAppointment"))) cancelMutation.mutate(appointment.id); }}>{t("registrations.cancelAppointment")}</Button></div></div> : <div className="rounded-2xl border border-border bg-muted/20 p-3 text-xs text-muted-foreground">{t("registrations.cancelNotAllowed")}</div> : null}
          ...
          {/*
        */}
        {mobileDocumentFooterVisible ? <div className="sticky bottom-0 z-30 grid shrink-0 grid-cols-[minmax(0,1.6fr)_minmax(0,1fr)] gap-2 border-t border-border bg-background/95 px-2 pt-2 backdrop-blur" style={{ paddingBottom: "max(0.5rem, env(safe-area-inset-bottom))" }}><Button type="button" size="sm" className="min-h-10 gap-1.5 text-xs" onClick={() => window.document.getElementById("request-documents-upload-file")?.click()}><Upload size={15} aria-hidden="true" />{t("documents.uploadRequest")}</Button><Button type="button" size="sm" variant="secondary" className="min-h-10 gap-1.5 text-xs" onClick={() => setActionMenuOpen(true)}><MoreHorizontal size={15} aria-hidden="true" />{t("requestScans.actions.more")}</Button></div> : null}
      ...
      <Dialog open={voidDialogOpen && Boolean(appointment)} onClose={() => { if (voidMutation.isPending) return; setVoidDialogOpen(false); setVoidReason(""); }}>
        <DialogContent maxWidth="min(94vw, 460px)" role="dialog" aria-labelledby="appointment-void-title" aria-describedby="appointment-void-description" className="!p-5">
          <DialogHeader closeLabel={t("toast.close")}>
            <DialogTitle id="appointment-void-title">{t("appointmentEditor.void")}</DialogTitle>
            <DialogDescription id="appointment-void-description">{t("appointmentEditor.voidConfirm")}</DialogDescription>
          </DialogHeader>
          <div className="mt-4"><label htmlFor="appointment-void-reason" className="mb-1 block text-xs font-semibold">{t("appointmentEditor.voidReasonPrompt")}</label><textarea id="appointment-void-reason" value={voidReason} onChange={(event) => setVoidReason(event.target.value)} rows={3} className="input-premium w-full resize-none" required /></div>
          <DialogFooter className="mt-5"><Button type="button" variant="secondary" disabled={voidMutation.isPending} onClick={() => { setVoidDialogOpen(false); setVoidReason(""); }}>{t("common.cancel")}</Button><Button type="button" variant="destructive" disabled={!voidReason.trim() || voidMutation.isPending} onClick={() => voidMutation.mutate()}>{voidMutation.isPending ? t("appointmentEditor.voiding") : t("appointmentEditor.void")}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
