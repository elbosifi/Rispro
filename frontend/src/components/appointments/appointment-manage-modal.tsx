...
import { inferSupportedOverrideType, inferSupportedOverrideTypeFromExamRuleMetadata } from "@/v2/appointments/utils/scheduling-override-requests";
...
export function AppointmentManageModal({
  ...
  const submitReschedule = () => {
    ...
    if (!payload) return;
    const supportedOverrideType = allowReceptionOverrideRequestsFromAvailability ? (inferSupportedOverrideTypeFromExamRuleMetadata({ requiresSupervisorOverride: Boolean(rescheduleSelectedRow?.requiresSupervisorOverride), effectModes: rescheduleSelectedRow?.matchedExamRuleSummary ? [rescheduleSelectedRow.matchedExamRuleSummary.effectMode] : [] }) ?? inferSupportedOverrideType(rescheduleSelectedRow?.reasonCodes)) : null;
          ...
          {appointment && activeTab === "reschedule" ? (
            selectedCanReschedule ? <div className="absolute inset-2 z-20 overflow-y-auto rounded-xl border border-border bg-background p-3 shadow-xl sm:inset-3 sm:p-5"><div className="mb-3 flex items-center justify-between gap-2"><div><h4 className="text-sm font-semibold">{t("registrations.reschedule")}</h4><p className="mt-1 text-xs text-muted-foreground">{t("registrations.rescheduleHint")}</p></div><button type="button" className="rounded-lg p-2 text-muted-foreground hover:bg-muted" onClick={() => selectTab("documents")} aria-label={t("toast.close")}><X size={17} aria-hidden="true" /></button></div>
              {rescheduleAvailability.isError ? <div className="rounded-xl border border-red-200 bg-red-50 p-3 text-xs text-red-700">{(rescheduleAvailability.error as Error | undefined)?.message || t("registrations.rescheduleAvailabilityFailed")}</div> : <form className="space-y-3" onSubmit={(event) => { event.preventDefault(); submitReschedule(); }}>
                ...
                <div className={`flex flex-wrap gap-2 ${isRtl ? "justify-start" : "justify-end"}`}>{user?.role === "receptionist" && rescheduleSelectedRow && (inferSupportedOverrideTypeFromExamRuleMetadata({ requiresSupervisorOverride: rescheduleSelectedRow.requiresSupervisorOverride, effectModes: rescheduleSelectedRow.matchedExamRuleSummary ? [rescheduleSelectedRow.matchedExamRuleSummary.effectMode] : [] }) ?? inferSupportedOverrideType(rescheduleSelectedRow.reasonCodes)) ? <Button type="button" size="sm" variant="secondary" disabled={!canSubmitReschedule || createRescheduleOverrideRequest.isPending} onClick={() => { const payload = buildReschedulePayload(); if (!payload) return; setPendingReschedulePayload(payload); setRescheduleRequestOverrideType(inferSupportedOverrideTypeFromExamRuleMetadata({ requiresSupervisorOverride: rescheduleSelectedRow.requiresSupervisorOverride, effectModes: rescheduleSelectedRow.matchedExamRuleSummary ? [rescheduleSelectedRow.matchedExamRuleSummary.effectMode] : [] }) ?? inferSupportedOverrideType(rescheduleSelectedRow.reasonCodes)); setRescheduleRequestError(null); setRescheduleRequestOpen(true); }}>{t("overrideRequests.requestApproval")}</Button> : null}<Button type="submit" size="sm" disabled={!canSubmitReschedule}>{rescheduleMutation.isPending ? t("appointments.v2.rescheduling") : t("registrations.reschedule")}</Button></div>
              </form>}
