...
export function MriPrimaryScreeningBadges({ result, expected = true, compact = false }: MriPrimaryScreeningBadgesProps) {
  ...
  if (compact) {
    if (!result) {
      const tooltip = t(language, "appointments.create.safety.compactMissingTooltip");
      return expected ? (
        <span aria-label={tooltip} title={tooltip} className="inline-flex h-7 min-w-7 items-center justify-center rounded border-2 border-amber-500 px-0.5 text-[10px] font-bold leading-none text-amber-700">MR?</span>
      ) : null;
    }

    if (result === "no_known_implant_reported") {
      const tooltip = t(language, "appointments.create.safety.compactNoImplantTooltip");
      return <span aria-label={tooltip} title={tooltip} className="inline-flex h-7 w-7 items-center justify-center rounded-sm border-2 border-emerald-600 text-[10px] font-bold leading-none text-emerald-700">MR</span>;
    }

    const tooltip = t(language, "appointments.create.safety.compactReviewTooltip");
    return (
      <span aria-label={tooltip} title={tooltip} className="relative inline-flex h-7 w-7 items-center justify-center text-amber-600">
