...
import { AlertTriangle, CalendarPlus, ChevronRight, Pencil, Printer, X } from "lucide-react";
...
export function PatientDrawer({
  ...
  if (isLoading) {
    return (
      <div className="fixed inset-0 z-[70] bg-black/30" onClick={onClose} data-testid="patient-drawer-backdrop">
        <div
          className="fixed inset-y-0 right-0 z-[80] flex w-full max-w-md items-center justify-center border-l border-border bg-background shadow-xl"
  ...
  return (
    <div className="fixed inset-0 z-[70] bg-black/30" onClick={onClose} data-testid="patient-drawer-backdrop">
      <div
        className="fixed inset-y-0 right-0 z-[80] flex w-full max-w-md flex-col overflow-hidden border-l border-border bg-background shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
