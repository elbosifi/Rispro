...
async function mockShell(page: Page, role: "doctor" | "super_admin") {
  ...
  await page.route("http://127.0.0.1:5173/api/**", async (route) => {
    ...
    if (path === "/api/doctor/protocoling/appointments/42/history") return route.fulfill({ json: { pacsStatus: "available", historicalPacsIndexStatus: "ready", historicalPacsLastSuccessAt: null, canReconcilePatientIdentity: true, currentPatient: { id: 9, patientId: "NEW-9", name: "Current Patient", birthDate: "1990-01-02" }, items: [{ appointmentId: null, orthancStudyId: "study-old", studyInstanceUid: "1.2.3.4", accessionNumber: "OLD-ACC", date: "2024-01-02", time: null, modalities: ["CT"], description: "Historical CT", appointmentStatus: null, reportAvailable: false, source: "pacs_only", identityDiscrepancy: null, historicalPatientId: "OLD-9", historicalPatientName: "Old^Patient", historicalPatientBirthDate: "19800102", reconciliation: null }] } });
    if (path === "/api/doctor/protocoling/appointments/42/history/historical-candidates") return route.fulfill({ json: { historicalCandidates: [], historicalPacsIndexStatus: "ready", historicalPacsLastSuccessAt: null } });
...
test("renders the doctor reconciliation confirmation at desktop width", async ({ page }, testInfo) => {
  ...
  await page.getByRole("button", { name: "Reconcile patient identity" }).click();
  const dialog = page.getByRole("heading", { name: "Patient Identity Reconciliation" }).locator("xpath=ancestor::*[@role='dialog'][1]");
  await expect(dialog.getByText("Old^Patient")).toBeVisible();
  await expect(dialog.getByText("Patient name: Current Patient")).toBeVisible();
  await expect(dialog.getByRole("button", { name: "Reconcile patient identity" })).toBeDisabled();
